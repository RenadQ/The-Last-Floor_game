...
function HubTabs({ onStartGame, profile, setProfile, openSettings, openAchievements, charData, onEditCharacter }) {
  ...
  return (
    <View style={{ width: LW, height: LH, backgroundColor: '#070615' }}>
      <StatusBar hidden />
      <View style={{ flex: 1, paddingHorizontal: 18, paddingTop: 14, paddingBottom: 6 }}>
        {tab === 'home' && (
          <HomeScreen profile={profile} charData={charData} onPlay={onStartGame} />
        )}
      ...
      {/* START button calls onStartGame directly — never just switches tab */}
      <BottomNav tab={tab} onTab={setTab} onStart={onStartGame} />
    </View>
  );
}

function BottomNav({ tab, onTab, onStart }) {
  const { width: W, height: H } = useWindowDimensions();
  const LH = Math.min(W, H);
  const BTN_H = Math.round(Math.max(44, Math.min(52, LH * 0.125)));
    ...
    return (
      <TouchableOpacity
        ...
        style={{
          ...
          borderWidth: 1,
          borderColor: active ? 'rgba(25,195,125,0.35)' : 'rgba(255,255,255,0.10)',
          alignItems: 'center',
          justifyContent: 'center',
        }}
      >
        <Text style={{ color: active ? '#e8dcc8' : 'rgba(232,220,200,0.55)', fontFamily: 'monospace', fontSize: 12, letterSpacing: 1 }}>
          {label}
        </Text>
      </TouchableOpacity>
    );
  ...
  return (
    ...
    >
      ...
      {/* START — calls onStart which triggers the full game flow */}
      <TouchableOpacity
        onPress={onStart}
        activeOpacity={0.9}
        style={{
      ...
      >
        <Text style={{ color: '#03020a', fontFamily: 'monospace', fontWeight: '900', fontSize: 14, letterSpacing: 3 }}>
          START
        </Text>
      </TouchableOpacity>
...
function HomeScreen({ profile, charData, onPlay }) {
  const name = profile?.name?.trim() ? profile.name.trim() : 'Human';
  const avatar = charData ? <ChibiCharacter charData={charData} size={1.4} /> : <View style={{width:56,height:56,borderRadius:18,backgroundColor:'rgba(255,255,255,0.05)',borderWidth:1,borderColor:'rgba(255,255,255,0.12)'}}/>;

const [tipState, setTipState] = useState({ loading: true, text: '', source: '' });
...
const fallbackTips = [
  'Collect all 4 keys across the floors, then open the exit door on Floor 4.',
  'You can move between floors freely — but the exit needs ALL 4 keys.',
  'Double tap SPEED to sprint for 20 seconds.',
  'Freeze stops ghosts briefly — walk over it to auto-activate.',
  'During Dark Mode: STAY SILENT. Screaming attracts ghosts to your location.',
  'Shield blocks one hit. Collect it and arm it for Dark Mode.',
];
...
useEffect(() => {
  ...
  const run = async () => {
    try{
      // Load stats
      const rawStats = await storage.get(STATS_KEY);
      if(rawStats && alive){ try{ const j=JSON.parse(rawStats); if(j) setStats(s=>({...s,...j})); }catch(e){} }
    }catch(e){}
    try{
      setTipState({ loading: true, text: '', source: '' });
      const res = await fetch('https://api.quotable.io/random?maxLength=90');
      if(!res.ok) throw new Error('Bad response');
      const j = await res.json();
      const txt = (j?.content ?? '').toString().trim();
      if(!txt) throw new Error('Empty');
      if(alive) setTipState({ loading: false, text: txt, source: 'Quotable API' });
    }catch(e){
      const tip = fallbackTips[Math.floor(Math.random() * fallbackTips.length)];
      if(alive) setTipState({ loading: false, text: tip, source: 'Offline' });
    }
  };
  run();
  return () => { alive = false; };
...
const tip = tipState.loading ? 'Loading tip...' : tipState.text;
    ...
    >
      ...
      <View style={{ alignItems: 'center', paddingTop: 8, paddingBottom: 14 }}>
        <View style={{
          ...
          borderWidth: 1.5, borderColor: 'rgba(201,164,76,0.25)',
          justifyContent: 'center', alignItems: 'center', marginBottom: 12,
        }}>
          {charData ? <ChibiCharacter charData={charData} size={1.5} /> :
            <Text style={{ color: 'rgba(255,255,255,0.2)', fontFamily: 'monospace', fontWeight: '900', fontSize: 26 }}>?</Text>}
        </View>
        <Text style={{
          color: '#e8dcc8', fontFamily: 'monospace', fontWeight: '900',
          letterSpacing: 5, fontSize: 20, textAlign: 'center',
          textShadowColor: 'rgba(255,0,80,0.45)', textShadowRadius: 16,
        }}>
          THE LAST FLOOR
        </Text>
        <Text style={{ color: 'rgba(255,255,255,0.32)', fontFamily: 'monospace', fontSize: 10, lineHeight: 16, textAlign: 'center', marginTop: 6 }}>
          4 floors · 4 keys · 1 exit · survive
        </Text>
      </View>
      ...
      <View style={{
        ...
        backgroundColor: 'rgba(0,0,0,0.35)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)',
        flexDirection: 'row', alignItems: 'center', gap: 14, marginBottom: 12,
      }}>
        <View style={{
          width: 46, height: 46, borderRadius: 14,
          backgroundColor: 'rgba(25,195,125,0.10)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.10)',
          justifyContent: 'center', alignItems: 'center', flexShrink: 0,
        }}>
          {charData ? <ChibiCharacter charData={charData} size={0.95} /> : null}
        </View>
        <View style={{ flex: 1 }}>
          <Text style={{ color: '#d6c8ff', fontFamily: 'monospace', fontWeight: '900', letterSpacing: 2, fontSize: 13 }}>
            Welcome, {name}
          </Text>
          <Text style={{ color: 'rgba(255,255,255,0.35)', fontFamily: 'monospace', fontSize: 10, marginTop: 5, lineHeight: 15 }}>
            Escapes: {stats.escapes}  ·  Deaths: {stats.deaths}  ·  Best: {msToClock(stats.bestEscapeMs)}
          </Text>
        </View>
      </View>
      ...
      <View style={{
        ...
        <Text style={{ color: 'rgba(201,164,76,0.75)', fontFamily: 'monospace', fontWeight: '900', letterSpacing: 3, fontSize: 10, marginBottom: 8 }}>
          💡 TIP
        </Text>
        <Text style={{ color: 'rgba(255,255,255,0.60)', fontFamily: 'monospace', fontSize: 11, lineHeight: 18 }}>
          {tip}
        </Text>
        {tipState.source ? (
          <Text style={{ color: 'rgba(255,255,255,0.22)', fontFamily: 'monospace', fontSize: 9, marginTop: 8 }}>
            Source: {tipState.source}
          </Text>
        ) : null}
      </View>
      ...
      <View style={{ flexDirection: 'row', gap: 8 }}>
        {[
          { label: 'ESCAPES', value: stats.escapes, color: '#c9a44c' },
          { label: 'DEATHS',  value: stats.deaths,  color: '#ff6666' },
          { label: 'BEST',    value: msToClock(stats.bestEscapeMs), color: '#88ddff' },
        ].map(s => (
          <View key={s.label} style={{
            flex: 1, backgroundColor: 'rgba(0,0,0,0.38)', borderRadius: 14,
            padding: 12, borderWidth: 1, borderColor: 'rgba(255,255,255,0.07)', alignItems: 'center', gap: 4,
          }}>
            <Text style={{ color: s.color, fontFamily: 'monospace', fontWeight: '900', fontSize: 17, lineHeight: 20 }}>
              {s.value}
            </Text>
            <Text style={{ color: 'rgba(255,255,255,0.35)', fontFamily: 'monospace', fontSize: 8, letterSpacing: 1 }}>
              {s.label}
            </Text>
          </View>
        ))}
...
function RulesScreen() {
  const Section = ({label})=>(
    <View style={{flexDirection:'row',alignItems:'center',gap:10,marginTop:22,marginBottom:10}}>
      ...
      <Text style={{color:'rgba(201,164,76,0.7)',fontFamily:'monospace',fontSize:10,letterSpacing:4}}>{label}</Text>
      <View style={{flex:1,height:1,backgroundColor:'rgba(255,255,255,0.07)'}}/>
    </View>
  );
  const Rule = ({sym,symColor='#d6c8ff',title,body})=>(
    <View style={{flexDirection:'row',gap:14,paddingVertical:13,paddingHorizontal:14,borderRadius:14,
      backgroundColor:'rgba(255,255,255,0.025)',borderWidth:1,borderColor:'rgba(255,255,255,0.07)',marginBottom:9}}>
      <View style={{width:44,height:44,borderRadius:12,backgroundColor:'rgba(0,0,0,0.4)',
        borderWidth:1,borderColor:'rgba(255,255,255,0.08)',alignItems:'center',justifyContent:'center'}}>
        <Text style={{color:symColor,fontFamily:'monospace',fontWeight:'900',fontSize:14}}>{sym}</Text>
      </View>
      <View style={{flex:1}}>
        <Text style={{color:symColor,fontFamily:'monospace',fontWeight:'900',fontSize:13,letterSpacing:2,marginBottom:5}}>{title}</Text>
        <Text style={{color:'rgba(255,255,255,0.55)',fontFamily:'monospace',fontSize:12,lineHeight:19}}>{body}</Text>
      </View>
    </View>
  );

  // Difficulty table data
  const DIFF_ROWS = [
    {label:'EASY',   color:'#00ff88', bg:'rgba(0,220,120,0.08)',  border:'rgba(0,220,120,0.30)',
     maze:'Small',  ghosts:'2 Normal', zone:'Small',  key:'Near',    items:'Freeze · Push×2 · Bandage · Speed'},
    {label:'MEDIUM', color:'#ffdd00', bg:'rgba(255,200,0,0.08)',  border:'rgba(255,200,0,0.30)',
     maze:'Medium', ghosts:'3 Normal', zone:'Medium', key:'Moderate',items:'Freeze · Push×2 · Bandage · Shield'},
    {label:'HARD',   color:'#ff8800', bg:'rgba(255,100,0,0.08)',  border:'rgba(255,100,0,0.30)',
     maze:'Large',  ghosts:'4 Normal', zone:'Large',  key:'Far',     items:'Freeze · Push×1 · Bandage · Shield · Kill Orb'},
    {label:'BRUTAL', color:'#ff4444', bg:'rgba(255,0,0,0.10)',    border:'rgba(255,0,0,0.35)',
     maze:'Huge',   ghosts:'2N + 2 Fast', zone:'Huge', key:'Very Far',items:'Freeze · Push×1 · Shield · Kill Orb'},
  ];

  return (
    <ScrollView style={{flex:1}} contentContainerStyle={{paddingBottom:40,paddingHorizontal:14}}
      showsVerticalScrollIndicator={false}>

      {/* Header */}
      <View style={{alignItems:'center',marginBottom:20,paddingTop:8}}>
        <Text style={{color:'rgba(255,255,255,0.2)',fontFamily:'monospace',fontSize:9,letterSpacing:6,marginBottom:6}}>THE LAST FLOOR</Text>
        <Text style={{color:'#e8dcc8',fontFamily:'monospace',fontWeight:'900',letterSpacing:6,fontSize:20}}>HOW TO PLAY</Text>
        <View style={{width:40,height:2,backgroundColor:'#c9a44c',borderRadius:1,marginTop:10,opacity:0.6}}/>
      </View>

      <Section label="OBJECTIVE"/>
      <View style={{padding:16,borderRadius:16,backgroundColor:'rgba(201,164,76,0.06)',borderWidth:1,borderColor:'rgba(201,164,76,0.2)',marginBottom:4}}>
        <Text style={{color:'rgba(255,255,255,0.78)',fontFamily:'monospace',fontSize:13,lineHeight:21}}>
          Survive all <Text style={{color:'#c9a44c',fontWeight:'900'}}>4 floors</Text>. Each floor has one key — find it. You can ride the elevator freely between floors, but to <Text style={{color:'#c9a44c',fontWeight:'900'}}>open the exit door</Text>, you must collect <Text style={{color:'#ff6666',fontWeight:'900'}}>all 4 keys</Text>. Once you have all 4 keys, go to Floor 4 and press <Text style={{color:'#80ffb0',fontWeight:'900'}}>OPEN THE DOOR</Text> to escape.
        </Text>
      </View>

      {/* ── DIFFICULTY TABLE — stacked card layout, works on any screen ── */}
      <Section label="DIFFICULTY TIERS"/>
      <View style={{gap:8,marginBottom:4}}>
        {DIFF_ROWS.map((row)=>(
          <View key={row.label} style={{
            borderRadius:14,borderWidth:1.5,borderColor:row.border,
            backgroundColor:row.bg,overflow:'hidden',
          }}>
            {/* Row 1: badge + 3 stats */}
            <View style={{flexDirection:'row',alignItems:'center',paddingHorizontal:12,paddingTop:10,paddingBottom:8,gap:8}}>
              {/* Tier badge */}
              <View style={{width:72,paddingVertical:5,borderRadius:8,alignItems:'center',
                backgroundColor:'rgba(0,0,0,0.35)',borderWidth:1.5,borderColor:row.border}}>
                <Text style={{fontFamily:'monospace',fontWeight:'900',fontSize:12,letterSpacing:2,color:row.color}}>{row.label}</Text>
              </View>
              {/* Stats */}
              <View style={{flex:1,flexDirection:'row',justifyContent:'space-around'}}>
                {[['MAZE',row.maze],['GHOSTS',row.ghosts],['KEY',row.key]].map(([lbl,val])=>(
                  <View key={lbl} style={{alignItems:'center',gap:2}}>
                    <Text style={{color:'rgba(255,255,255,0.30)',fontFamily:'monospace',fontSize:8,letterSpacing:1}}>{lbl}</Text>
                    <Text style={{color:'rgba(255,255,255,0.80)',fontFamily:'monospace',fontWeight:'900',fontSize:11}}>{val}</Text>
                  </View>
                ))}
              </View>
            </View>
            ...
            <View style={{borderTopWidth:1,borderTopColor:'rgba(255,255,255,0.07)',
              ...
              <Text style={{color:'rgba(255,255,255,0.28)',fontFamily:'monospace',fontSize:8,letterSpacing:2,width:36}}>ITEMS</Text>
              <Text style={{flex:1,fontFamily:'monospace',fontSize:11,color:row.color,lineHeight:17,flexWrap:'wrap'}}>{row.items}</Text>
            </View>
          </View>
        ))}
      </View>

      <Section label="SILENCE WARNING"/>
      <View style={{padding:16,borderRadius:16,backgroundColor:'rgba(255,30,30,0.07)',borderWidth:1.5,borderColor:'rgba(255,80,80,0.35)',marginBottom:4}}>
        <Text style={{color:'#ff8888',fontFamily:'monospace',fontWeight:'900',fontSize:13,letterSpacing:2,marginBottom:7}}>STAY SILENT — GHOSTS CAN HEAR YOU</Text>
        <Text style={{color:'rgba(255,200,200,0.78)',fontFamily:'monospace',fontSize:12,lineHeight:20}}>
          During <Text style={{color:'#aa88ff',fontWeight:'900'}}>Dark Mode</Text>, the microphone is active. If you scream, shout, or make loud sounds, any ghost nearby will immediately rush toward you. Stay quiet — your voice is your worst enemy.
        </Text>
      </View>

      <Section label="MOVEMENT & ACTIONS"/>
      <Rule sym="KEY" symColor="#c9a44c" title="Collect the Key" body="Stand near the key and hold CAPTURE (5 seconds). Every floor has exactly one key. You MUST collect the key before the elevator will take you to the next floor."/>
      <Rule sym="UP" symColor="#a090ff" title="Use the Elevator" body="Stand inside the elevator zone and press ELEVATOR UP. Ride freely between floors at any time. On Floor 4, once you have all 4 keys, the button becomes OPEN THE DOOR — press it to escape."/>
      <Rule sym="EXIT" symColor="#80ffb0" title="Open The Door" body="On Floor 4, once you have all 4 floor keys, step into the elevator and press OPEN THE DOOR to win and escape the building."/>
      <Rule sym="SPD" symColor="#aaffaa" title="Sprint" body="Double-tap the SPEED button to sprint for 20 seconds. Great for escaping a chase."/>

      <Section label="ITEMS (find in maze)"/>
      <Rule sym="FRZ" symColor="#88ddff" title="Freeze Charge" body="Walk over to collect (1 per floor). Press FREEZE to activate — lights turn on for 20 seconds and all ghosts vanish so you can move freely."/>
      <Rule sym="PSH" symColor="#ff8844" title="Push Charge" body="Tap PUSH when a ghost is close to eliminate it instantly. Find extra charges in the maze."/>
      <Rule sym="BND" symColor="#88ffaa" title="Bandage" body="Hold CAPTURE near a bandage to restore 1 heart. Not available on BRUTAL difficulty."/>
      <Rule sym="SHD" symColor="#44ccff" title="Shield" body="Absorbs one ghost hit entirely. Arm it before entering a danger zone. Available on MEDIUM, HARD and BRUTAL."/>
      <Rule sym="KLL" symColor="#ff88ff" title="Kill Orb" body="Hold CAPTURE to eliminate the nearest ghost permanently. Found on HARD and BRUTAL floors."/>

      <Section label="GHOSTS & DANGER"/>
      <Rule sym="!!!" symColor="#ff4455" title="Ghost Zones" body="Each ghost patrols a circular zone. Step inside and the ghost notices you — stay too long and it gives chase."/>
      <Rule sym="RUN" symColor="#ff2244" title="Being Chased" body="If a ghost locks onto you, sprint and break line-of-sight. Getting caught costs 1 heart."/>
      <Rule sym="DRK" symColor="#aa88ff" title="Dark Mode" body="After a warning countdown, lights cut out. Ghosts become aggressive and chase you everywhere. Use freeze or sprint — and stay SILENT. Any loud sound will attract nearby ghosts directly to you."/>
      <Rule sym="FST" symColor="#ff88cc" title="Fast Ghost" body="Found on BRUTAL floors. Moves 1.5× faster than normal ghosts when patrolling and 1.55× faster when chasing. Prioritize freeze or push charges against them."/>

      <Section label="TIPS"/>
      <View style={{gap:8,marginBottom:4}}>
        {[
          'You can ride the elevator between floors freely — but you need ALL 4 keys to open the final door.',
          'Floors are random every run — you never know what difficulty comes next.',
          'Explore during the 15s lit preview — memorize where the key and elevator are.',
          'During Dark Mode, stay completely silent — loud sounds bring ghosts rushing to you.',
          'Freeze is best saved for a chase, not just whenever you find it.',
          'Push charges, shield and hearts carry over between floors — use them wisely.',
          'No bandage on the hardest floors — shield and kill orb are your only safety net.',
        ].map((t,i)=>(
          <View key={i} style={{flexDirection:'row',gap:10,paddingHorizontal:4}}>
            <Text style={{color:'rgba(201,164,76,0.7)',fontFamily:'monospace',fontSize:13,marginTop:1}}>›</Text>
            <Text style={{color:'rgba(255,255,255,0.55)',fontFamily:'monospace',fontSize:12,lineHeight:20,flex:1}}>{t}</Text>
          </View>
        ))}
      </View>
...
function ProfileScreen({ profile, setProfile, charData, onEditCharacter, onPlay, openSettings, openAchievements }) {
  ...
  const StatCard = ({symbol, label, value, color='#d6c8ff'}) => (
    <View style={{flex:1, backgroundColor:'rgba(0,0,0,0.38)', borderRadius:14, padding:12, borderWidth:1, borderColor:'rgba(255,255,255,0.08)', alignItems:'center', gap:4}}>
      <Text style={{color:'rgba(255,255,255,0.25)', fontFamily:'monospace', fontWeight:'900', fontSize:11, letterSpacing:2}}>{symbol}</Text>
      <Text style={{color, fontFamily:'monospace', fontWeight:'900', fontSize:16, lineHeight:18}}>{value}</Text>
      <Text style={{color:'rgba(255,255,255,0.38)', fontFamily:'monospace', fontSize:8, letterSpacing:1}}>{label}</Text>
    </View>
  );

  const ActionRow = ({symbol, symColor, title, sub, titleColor, onPress, borderColor, bgColor}) => (
    <TouchableOpacity onPress={onPress} activeOpacity={0.88}
      ...
      <View style={{width:36, height:36, borderRadius:10, backgroundColor:'rgba(0,0,0,0.3)', borderWidth:1, borderColor:'rgba(255,255,255,0.08)', alignItems:'center', justifyContent:'center'}}>
        <Text style={{color:symColor, fontFamily:'monospace', fontWeight:'900', fontSize:12, letterSpacing:1}}>{symbol}</Text>
      </View>
      <View style={{flex:1}}>
        <Text style={{color:titleColor, fontFamily:'monospace', fontWeight:'900', fontSize:12, letterSpacing:1}}>{title}</Text>
        <Text style={{color:'rgba(255,255,255,0.35)', fontFamily:'monospace', fontSize:9, marginTop:2}}>{sub}</Text>
      </View>
      <Text style={{color:'rgba(255,255,255,0.25)', fontFamily:'monospace', fontSize:18}}>{">"}</Text>
    </TouchableOpacity>
  );

  return (
    <ScrollView style={{flex:1}} showsVerticalScrollIndicator={false}>
      <View style={{borderRadius:20, backgroundColor:'rgba(255,255,255,0.02)', borderWidth:1, borderColor:'rgba(255,255,255,0.09)', overflow:'hidden', marginBottom:12}}>
        <View style={{height:3, backgroundColor:'#c9a44c', opacity:0.6}}/>
        <View style={{padding:18, alignItems:'center', gap:8}}>
          <View style={{width:80, height:80, borderRadius:24, backgroundColor:'rgba(0,0,0,0.4)', borderWidth:2, borderColor:'rgba(201,164,76,0.35)', justifyContent:'center', alignItems:'center'}}>
            {charData ? <ChibiCharacter charData={charData} size={1.4}/> : (
              <Text style={{color:'rgba(255,255,255,0.2)', fontFamily:'monospace', fontWeight:'900', fontSize:22}}>?</Text>
            )}
          </View>
          {editing ? (
            <View style={{flexDirection:'row', gap:8, alignItems:'center', width:'100%'}}>
              <TextInput value={name} onChangeText={setName} autoFocus
                style={{flex:1, color:'white', fontFamily:'monospace', fontWeight:'900', fontSize:18, textAlign:'center',
                  backgroundColor:'rgba(255,255,255,0.06)', borderRadius:10, paddingHorizontal:12, paddingVertical:7,
                  borderWidth:1.5, borderColor:'#c9a44c'}}/>
              <TouchableOpacity onPress={save} style={{backgroundColor:'#c9a44c', borderRadius:10, paddingHorizontal:14, paddingVertical:9}}>
                <Text style={{color:'#000', fontFamily:'monospace', fontWeight:'900', fontSize:11}}>SAVE</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={()=>setEditing(false)} style={{backgroundColor:'rgba(255,255,255,0.08)', borderRadius:10, paddingHorizontal:10, paddingVertical:9}}>
                <Text style={{color:'rgba(255,255,255,0.6)', fontFamily:'monospace', fontSize:11}}>X</Text>
              </TouchableOpacity>
            </View>
          ) : (
            <TouchableOpacity onPress={()=>setEditing(true)} style={{flexDirection:'row', alignItems:'center', gap:8}}>
              <Text style={{color:'#e8dcc8', fontFamily:'monospace', fontWeight:'900', fontSize:20, letterSpacing:2}}>{name||'Human'}</Text>
              <View style={{borderWidth:1, borderColor:'rgba(201,164,76,0.5)', borderRadius:6, paddingHorizontal:6, paddingVertical:2}}>
                <Text style={{color:'rgba(201,164,76,0.8)', fontFamily:'monospace', fontSize:8, letterSpacing:1}}>EDIT</Text>
              </View>
            </TouchableOpacity>
          )}
          {!!msg&&<Text style={{color:'#19C37D', fontFamily:'monospace', fontSize:10, letterSpacing:2}}>{msg}</Text>}
          <Text style={{color:'rgba(255,255,255,0.25)', fontFamily:'monospace', fontSize:9, letterSpacing:2}}>
            {stats.escapes>5?'VETERAN SURVIVOR':stats.escapes>2?'RISING ESCAPIST':stats.escapes>0?'FLOOR RUNNER':'NEWCOMER'}
          </Text>
        </View>
      </View>

      <Text style={{color:'rgba(255,255,255,0.30)', fontFamily:'monospace', fontSize:9, letterSpacing:3, marginBottom:8}}>STATISTICS</Text>
      <View style={{flexDirection:'row', gap:8, marginBottom:8}}>
        <StatCard symbol="WIN" label="ESCAPES" value={stats.escapes} color='#c9a44c'/>
        <StatCard symbol="RIP" label="DEATHS" value={stats.deaths} color='#ff6666'/>
        <StatCard symbol="PCT" label="WIN RATE" value={`${winRate}%`} color='#19C37D'/>
      </View>
      <View style={{flexDirection:'row', gap:8, marginBottom:14}}>
        <StatCard symbol="TIME" label="BEST TIME" value={msToClock(stats.bestEscapeMs)} color='#88ddff'/>
        <StatCard symbol="ALL" label="TOTAL RUNS" value={stats.totalRuns||stats.escapes+stats.deaths||0} color='#d6c8ff'/>
      </View>

      <View style={{gap:9, marginBottom:14}}>
        <ActionRow symbol="CHR" symColor='#c9a44c' title="EDIT CHARACTER" sub="Change your character selection"
          titleColor='#c9a44c' onPress={onEditCharacter} borderColor='rgba(201,164,76,0.4)' bgColor='rgba(201,164,76,0.08)'/>
        <ActionRow symbol="ACH" symColor='#b0a0ff' title="ACHIEVEMENTS" sub="Track your milestones and unlocks"
          titleColor='#b0a0ff' onPress={openAchievements} borderColor='rgba(100,80,255,0.28)' bgColor='rgba(100,80,255,0.06)'/>
        <ActionRow symbol="SET" symColor='rgba(255,255,255,0.65)' title="SETTINGS" sub="Audio, controls, accessibility"
          titleColor='rgba(255,255,255,0.75)' onPress={openSettings} borderColor='rgba(255,255,255,0.12)' bgColor='rgba(255,255,255,0.03)'/>
      </View>
    </ScrollView>
  );
}
...
function TypingIntro({ name, onDone }) {
  ...
  const line3Y    = useRef(new Animated.Value(30)).current;
  ...
  const animIn = (op, y, delay) => Animated.parallel([
    ...
    Animated.timing(y,  {toValue:0, duration:600, delay, easing:Easing.out(Easing.cubic), useNativeDriver:true}),
  ]);

  useEffect(()=>{
    // Sequence: bg → line1 → line2 → line3 → credits → pulse → done
    Animated.sequence([
      Animated.timing(bgFade, {toValue:1, duration:800, useNativeDriver:true}),
      animIn(line1Op, line1Y, 0),
      Animated.delay(300),
      animIn(line2Op, line2Y, 0),
      Animated.delay(300),
      animIn(line3Op, line3Y, 0),
      Animated.delay(200),
      Animated.timing(lineW, {toValue:1, duration:600, useNativeDriver:false}),
      Animated.delay(100),
      Animated.timing(credOp, {toValue:1, duration:800, useNativeDriver:true}),
      Animated.delay(1800),
    ]).start(()=>{ if(onDoneRef.current) onDoneRef.current(); });

    // Pulse the "ready" word
    const loop = ()=>Animated.sequence([
      Animated.timing(pulseAnim,{toValue:1.06,duration:800,useNativeDriver:true}),
      Animated.timing(pulseAnim,{toValue:1.0, duration:800,useNativeDriver:true}),
    ]).start(loop);
    const pt = setTimeout(loop, 1800);
  ...
  return(
    <Animated.View style={{
      ...
      <View style={{alignItems:'center', paddingHorizontal:Math.round(VW*0.05), gap:0}}>
        ...
        {/* Line 1 — greeting */}
        <Animated.Text style={{
          opacity:line1Op, transform:[{translateY:line1Y}],
          color:'rgba(255,255,255,0.35)', fontFamily:'monospace',
          fontSize:Math.round(Math.max(9,Math.min(12,VH*0.022))), letterSpacing:5, marginBottom:Math.round(VH*0.035),
        }}>
          {`WELCOME, ${(name||'SURVIVOR').toUpperCase()}`}
        </Animated.Text>
        ...
        <Animated.Text style={{
          opacity:line3Op, transform:[{translateY:line3Y}],
          ...
          {'4 floors. Real ghosts. No second chances.Find the key. Reach the elevator. Escape.'}
        </Animated.Text>
...
function NameSetup({ initialName, onSave, onBack }) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#070615' }}>
      <StatusBar hidden />
      <View style={{ flex: 1, padding: 18, justifyContent: 'center' }}>
        <View style={{ borderRadius: 18, borderWidth: 1, borderColor: 'rgba(255,255,255,0.10)', backgroundColor: 'rgba(255,255,255,0.02)', padding: 16 }}>
          <Text style={{ color: '#e8dcc8', fontFamily: 'monospace', fontWeight: '900', letterSpacing: 5, fontSize: 14, textAlign: 'center' }}>
            ENTER YOUR NAME
          </Text>
          ...
          <View style={{ flexDirection: 'row', gap: 10, marginTop: 14 }}>
            <TouchableOpacity onPress={onBack} activeOpacity={0.9} style={{ flex: 1, paddingVertical: 12, borderRadius: 14, backgroundColor: 'rgba(255,255,255,0.04)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.12)', alignItems: 'center' }}>
              <Text style={{ color: 'rgba(255,255,255,0.70)', fontFamily: 'monospace', fontWeight: '900', letterSpacing: 2 }}>BACK</Text>
            </TouchableOpacity>

            <TouchableOpacity
              onPress={() => onSave(can ? String(name).trim() : 'Human')}
              activeOpacity={0.9}
              style={{ flex: 1, paddingVertical: 12, borderRadius: 14, backgroundColor: can ? 'rgba(201,164,76,0.16)' : 'rgba(255,255,255,0.03)', borderWidth: 1.5, borderColor: can ? '#c9a44c' : 'rgba(255,255,255,0.12)', alignItems: 'center' }}
            >
              <Text style={{ color: can ? '#c9a44c' : 'rgba(255,255,255,0.35)', fontFamily: 'monospace', fontWeight: '900', letterSpacing: 2 }}>SAVE</Text>
            </TouchableOpacity>
          </View>
        </View>
      </View>
...
const ENEMY_R    = 28;
const GHOST_SPD_PATROL = 2.2;   // calm patrolling inside zone
...
// ── HOLD INTERACTIONS ─────────────────────────────────────────────────────────
// All collectable items require 5-second hold interaction to collect
// except Key is 10 seconds
const KEY_HOLD_MS        = 10000;
...
const SPRINT_DURATION_MS   = 20000;
...
function buildFloors(){
  ...
  // This means even the sequence in which you encounter floors is random each run.
  ...
  const floors = diffOrder.map((di, fi)=>{
    ...
    return{
      id:fi+1,
      diffIdx:di,
      diffLabel:cfg.diffLabel,   // 'EASY' | 'MEDIUM' | 'HARD' | 'BRUTAL'
      theme:themeOrder[fi],
      walls,
      elevator,
      escapeDoor,
...
function GhostSprite({ state, kind, zoom, isFrozen=false }) {
  ...
  const bodyColor  = isFrozen ? '#88ccff' : isPanic ? '#cc2288' : isFast&&state==='chase' ? '#ff0066' : state==='chase' ? '#ff1133' : state==='alert' ? '#ff8800' : isFast?'#ff88cc':'#c8c0e8';
  const glowColor  = isFrozen ? 'rgba(100,200,255,0.35)' : isPanic ? 'rgba(200,30,130,0.4)' : state==='chase' ? 'rgba(255,10,40,0.45)' : state==='alert' ? 'rgba(255,140,0,0.35)' : 'rgba(160,140,220,0.18)';
  const eyeColor   = isFrozen ? '#aaddff' : state==='chase' ? '#ff0000' : state==='alert' ? '#ffcc00' : '#0a0820';
  const r = ENEMY_R * zoom;

  // ── IMAGE GHOST MODE ───────────────────────────────────────────────────────
  const ghostImg = GHOST_IMAGE_REGISTRY[kind] ?? GHOST_IMAGE_REGISTRY['NORMAL'];
  if(ghostImg){
    const tint = isFrozen?'rgba(100,200,255,0.4)':state==='chase'?'rgba(255,30,50,0.25)':state==='alert'?'rgba(255,140,0,0.2)':undefined;
    return(
      <View style={{width:r*6.5,height:r*6.5,alignItems:'center',justifyContent:'center'}}>
        <View style={{position:'absolute',width:r*6.5,height:r*6.5,borderRadius:r*3.25,backgroundColor:glowColor}}/>
        <Image source={ghostImg} style={{width:r*6.0,height:r*6.0,opacity:isFrozen?0.7:0.95}} resizeMode="contain"/>
...
// ═════════════════════════════════════════════════════════════════════════════
//  IMAGE ASSETS — base64 embedded for Expo Snack / web compatibility
// ═════════════════════════════════════════════════════════════════════════════
const IMG_CHAR2 = require('./assets/char2.png');
const IMG_CHAR3 = require('./assets/char3.png');
const IMG_CHAR4 = require('./assets/char4.png');
const IMG_CHAR5 = require('./assets/char5.png');
const IMG_CHAR6 = require('./assets/char6.png');
const IMG_CHAR7 = require('./assets/char7.png');
const IMG_GHOST1 = require('./assets/ghost1.png');
const IMG_GHOST2 = require('./assets/ghost2.png');
const IMG_GHOST3 = require('./assets/ghost3.png');
...
function CharacterCreator({ onDone, uiMsg = { text:'', until:0 }, theme = THEMES[0] }){
  ...
  const ROWS = [CHARACTER_REGISTRY.slice(0,3), CHARACTER_REGISTRY.slice(3,6)];
...
//  DEAD / WIN SCREENS
// ─────────────────────────────────────────────────────────────────────────────
function DeadScreen({charData,onRetry,onMenu,floorNum=1}){
  const fade   = useRef(new Animated.Value(0)).current;
  ...
  const tips=[
    'Stay outside the zone rings — ghosts notice intruders.',
    'Use FREEZE to move freely for 20 seconds.',
    'PUSH charges can save you from a chase.',
    'Arm your SHIELD before entering dangerous zones.',
    'Check the 15s preview — plan your route first.',
  ];
  const tip = tips[(floorNum-1) % tips.length];
  return(
    <View style={{flex:1,backgroundColor:'#060004'}}>
      <StatusBar hidden/>
      {/* Dark red vignette */}
      <View style={{position:'absolute',left:0,right:0,top:0,bottom:0,
        backgroundColor:'rgba(180,0,20,0.08)'}} pointerEvents="none"/>
      {/* Top blood line */}
      ...
      <Animated.View style={{flex:1,opacity:fade,transform:[{translateY:slideY}],
        ...
        {/* Character — faded, falling */}
        <Animated.View style={{transform:[{scale:sc}],marginBottom:16,opacity:0.32}}>
          {charData&&<ChibiCharacter charData={charData} size={2.8}/>}
        </Animated.View>

        {/* Title */}
        <Animated.Text style={{
          fontSize:52,fontWeight:'900',color:'#cc1122',fontFamily:'monospace',letterSpacing:6,
          textShadowColor:glowA.interpolate({inputRange:[0,1],outputRange:['rgba(200,0,20,0)','rgba(200,0,20,0.55)']}),
          textShadowRadius:28, marginBottom:6,
        }}>DEFEATED</Animated.Text>

        <Text style={{color:'rgba(255,255,255,0.35)',fontFamily:'monospace',fontSize:10,letterSpacing:4,marginBottom:20}}>
          FLOOR {floorNum}  ·  CAUGHT BY A GHOST
        </Text>

        {/* Divider */}
        <View style={{width:'60%',height:1,backgroundColor:'rgba(200,0,20,0.3)',marginBottom:20}}/>
        ...
        <View style={{
          ...
          maxWidth:340,width:'100%',marginBottom:28,
        }}>
          <Text style={{color:'rgba(201,164,76,0.7)',fontFamily:'monospace',fontSize:8,letterSpacing:4,marginBottom:6}}>SURVIVAL TIP</Text>
          <Text style={{color:'rgba(255,255,255,0.55)',fontFamily:'monospace',fontSize:11,lineHeight:18}}>{tip}</Text>
        </View>

        {/* Buttons */}
        <View style={{gap:10,width:'100%',maxWidth:340}}>
          <TouchableOpacity
            onPress={onRetry} activeOpacity={0.85}
            style={{
              borderWidth:1.5,borderColor:'#cc2233',
              paddingVertical:16,borderRadius:12,
              backgroundColor:'rgba(200,20,40,0.14)',
              alignItems:'center',
            }}>
            <Text style={{color:'#ff4455',fontFamily:'monospace',fontWeight:'900',letterSpacing:4,fontSize:13}}>
              RETRY FLOOR {floorNum}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={onMenu} activeOpacity={0.85}
            style={{
            ...
            <Text style={{color:'rgba(255,255,255,0.55)',fontFamily:'monospace',letterSpacing:3,fontSize:12}}>
              BACK TO HOME
            </Text>
          </TouchableOpacity>
        </View>
...
function WinScreen({charData,onPlay,onMenu,durMs=0,bestEscapeMs=0,brokeRecord=false,escapes=0}){
  ...
  const msToClock=(ms)=>{if(!ms||ms<=0)return'--';const t=Math.floor(ms/1000);const m=Math.floor(t/60);const s=t%60;return`${m}:${s<10?'0':''}${s}`;};
  return(
    <View style={{flex:1,backgroundColor:'#020b06',justifyContent:'center',alignItems:'center',padding:24}}>
      <StatusBar hidden/>
      <Animated.View style={{opacity:fade,alignItems:'center',width:'100%',maxWidth:380}}>
        {/* Gold accent line */}
        <View style={{width:80,height:3,borderRadius:2,backgroundColor:'#c9a44c',marginBottom:18}}/>
        <Animated.View style={{transform:[{scale:sc}],marginBottom:12}}>
          {charData&&<ChibiCharacter charData={charData} size={3.2}/>}
        </Animated.View>
        <Animated.Text style={{fontSize:38,fontWeight:'900',color:'#5ecfa0',fontFamily:'monospace',letterSpacing:6,
          textShadowColor:glowAnim.interpolate({inputRange:[0,1],outputRange:['rgba(94,207,160,0.0)','rgba(94,207,160,0.6)']}),
          textShadowRadius:24}}>
          🎉 ESCAPED!
        </Animated.Text>
        <Text style={{color:'rgba(255,255,255,0.55)',fontFamily:'monospace',fontSize:11,letterSpacing:2,marginTop:6,textAlign:'center'}}>
          All 4 floors conquered. You survived.
        </Text>

        {/* Score Card */}
        <View style={{marginTop:20,width:'100%',borderRadius:12,borderWidth:1,borderColor:'rgba(94,207,160,0.25)',backgroundColor:'rgba(94,207,160,0.06)',padding:16,gap:8}}>
          <Text style={{color:'rgba(255,255,255,0.35)',fontFamily:'monospace',fontSize:9,letterSpacing:3}}>YOUR SCORE</Text>
          <View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>
            <Text style={{color:'rgba(255,255,255,0.65)',fontFamily:'monospace',fontSize:11}}>This run</Text>
            <Text style={{color:'#5ecfa0',fontFamily:'monospace',fontWeight:'900',fontSize:13}}>{msToClock(durMs)}</Text>
          </View>
          <View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>
            <Text style={{color:'rgba(255,255,255,0.65)',fontFamily:'monospace',fontSize:11}}>Best time</Text>
            <Text style={{color:'#c9a44c',fontFamily:'monospace',fontWeight:'900',fontSize:13}}>{msToClock(bestEscapeMs)}</Text>
          </View>
          <View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>
            <Text style={{color:'rgba(255,255,255,0.65)',fontFamily:'monospace',fontSize:11}}>Total escapes</Text>
            <Text style={{color:'#d6c8ff',fontFamily:'monospace',fontWeight:'900',fontSize:13}}>{escapes}</Text>
          </View>
          {brokeRecord&&(
            <View style={{marginTop:4,paddingVertical:6,paddingHorizontal:10,borderRadius:6,backgroundColor:'rgba(201,164,76,0.15)',borderWidth:1,borderColor:'rgba(201,164,76,0.35)',alignItems:'center'}}>
              <Text style={{color:'#c9a44c',fontFamily:'monospace',fontWeight:'900',fontSize:10,letterSpacing:3}}>★ NEW RECORD!</Text>
            </View>
          )}
        </View>

        {/* Buttons */}
        <View style={{marginTop:22,gap:12,width:'100%'}}>
          <TouchableOpacity
            style={{borderWidth:1.5,borderColor:'#5ecfa0',paddingVertical:15,paddingHorizontal:28,borderRadius:8,backgroundColor:'rgba(94,207,160,0.12)',alignItems:'center'}}
            onPress={onPlay} activeOpacity={0.8}>
            <Text style={{color:'#5ecfa0',fontFamily:'monospace',letterSpacing:4,fontSize:13,fontWeight:'900'}}>▶ PLAY AGAIN</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={{borderWidth:1,borderColor:'rgba(255,255,255,0.18)',paddingVertical:14,paddingHorizontal:28,borderRadius:8,backgroundColor:'rgba(255,255,255,0.04)',alignItems:'center'}}
            onPress={onMenu} activeOpacity={0.8}>
            <Text style={{color:'rgba(255,255,255,0.60)',fontFamily:'monospace',letterSpacing:3,fontSize:12}}>⌂ MAIN MENU</Text>
          </TouchableOpacity>
        </View>
      </Animated.View>
    </View>
...
function GameCore({ navigation }){
  ...
  const HUD_H      = Math.round(Math.max(42, Math.min(50, SH * 0.110)));
  const HINT_H     = Math.round(Math.max(22, Math.min(28, SH * 0.055)));
  const ACT_H      = 32;
  const SAFE_BOT   = Platform.OS === 'ios' ? 14 : 4;
  const CONTROLS_H = ACT_H * 3 + SAFE_BOT + 12;
  // ZOOM: width-fit — maze fills 100% of screen width; camera scrolls vertically
  const ZOOM=useMemo(()=>{
  ...
  const loadStats = useCallback(async()=>{
    try{
      ...
      if(raw){
        ...
        if(j && typeof j === 'object') setStats(s => ({...s, ...j}));
      }
    }catch(e){/* ignore */}
  },[]);
...
const[charData,setCharData]=useState(null);
  ...
  useEffect(()=>{(async()=>{
    ...
    if(raw){try{const p=JSON.parse(raw);profileRef.current=p;setProfile(p);}catch(e){}}
  ...
  const [inv,setInv]=useState({
    ...
    bandagesCollected:0,
    pushCharges:2,
    freezeCharges:0,  // gained by walking over freeze pickup; 1 per floor
  });
  const invRef=useRef({
    speedUnlocked:false,
    shieldCharges:0,
...
// ── SPRINT: double tap arrows starts sprint for 30 seconds ──────────────────
...
const pushGhost = useCallback(()=>{
  ...
  showMsg(`💥 Ghost eliminated! ${next} push${next!==1?'es':''} left`);
},[ showMsg]);
...
const activateFreeze = useCallback(()=>{
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      await safePlay(amb);await safePlay(pnc);
    })();
  ...
  useEffect(()=>{
    ...
    const tick=()=>{
      ...
      for(const e of enemiesRef.current){
        // ── Dark mode: ghosts ignore zones and hard-chase ──
        if(darkActive && e.kind!=='PANIC'){
          e.state='chase';
          e.alertUntil=now+GHOST_ALERT_LINGER;
        }
        // ── Mic lure (only matters during dark mode) ──
        else if(loud&&dist(pl.x,pl.y,e.x,e.y)<=MIC_LURE_RADIUS){
          e.state='chase';
          e.alertUntil=now+GHOST_ALERT_LINGER;
        ...
        if(e.state==='chase'){
          tx=pl.x;ty=pl.y;
          const fastMult=e.kind==='FAST'?1.55:1;
          ...
          const t2=e.patrol[e.patrolIndex];tx=t2.x;ty=t2.y;spd=GHOST_SPD_PATROL*(e.kind==='FAST'?1.5:1);
        }
      ...
      if(canUseAV && now-lastAudioCheckRef.current>80){
        ...
        const nearGhost=enemiesRef.current.some(e=>dist(pl.x,pl.y,e.zone.cx,e.zone.cy)<e.zone.r+GHOST_VOICE_RANGE);
        ...
        if(!stillNear){cancelCapture();}
          ...
          if(progress01>=1){
            if(cap.type==='KEY'){
              ...
              setInvSafe({ speedUnlocked:true });
              showMsg('⚡ Speed Power unlocked. Double tap to sprint.');
  ...
  const resetAll=useCallback(()=>{
    ...
    setInv({speedUnlocked:false,shieldCharges:0,shieldArmed:false,bandagesCollected:0,pushCharges:2,freezeCharges:0});
    invRef.current={speedUnlocked:false,shieldCharges:0,shieldArmed:false,bandagesCollected:0,pushCharges:2,freezeCharges:0};
  ...
  const finishRun = useCallback(async(outcome, extra={})=>{
    ...
    next.totalRuns = (prev.totalRuns || 0) + 1;  // track every run
    ...
    try{
      await storage.set('LASTFLOOR_LASTRESULT_V1', JSON.stringify({ outcome, durMs: dur, at: endAt, ...extra }));
    }catch(e){/* ignore */}
    ...
    setScreen(outcome === 'win' ? 'win' : 'dead');
  },[navigation, saveStats, stats]);
  ...
  const elevatorSequence=useCallback(async()=>{
    ...
    try{
      ...
      enemiesRef.current=nextFloor.enemies.map(e=>({...e,patrolIndex:0,frozenUntil:0}));
  ...
  useEffect(()=>{
    if(screen==='play'){
      ...
      enemiesRef.current=f.enemies.map(e=>({...e,patrolIndex:0,frozenUntil:0}));
  ...
  if(screen==='intro') return <IntroVideoScreen onDone={()=>{ setScreen(boot?'loading':'hub'); }} />;
  // 2. Loading splash (while boot timer runs, then auto-goes to hub)
  if(screen==='loading'||boot) return <LaunchScreen onReady={()=>setScreen('hub')} bootDone={!boot}/>;
  ...
  if(screen==='dead')return<DeadScreen charData={charData} floorNum={floorIndex+1} onRetry={()=>{
    // Restart from the SAME floor, reset only current floor state
    const f=floorsRef.current[floorIndex];
    enemiesRef.current=f.enemies.map(e=>({...e,patrolIndex:0,frozenUntil:0}));
    playerRef.current={x:f.spawn.x,y:f.spawn.y};
    setPlayer({x:f.spawn.x,y:f.spawn.y});
    // Reset key for this floor only
    setKeysTaken(prev=>{const next=[...prev];next[floorIndex]=false;keysTakenRef.current=next;return next;});
    const fh=FLOOR_START_HEARTS[floorIndex]??3;
    floorMaxHeartsRef.current=fh;setFloorMaxHearts(fh);heartsRef.current=fh;setHearts(fh);
    captureRef.current=null;setCaptureUI({visible:false,type:null,progress01:0});
...
// ── PLAY ─────────────────────────────────────────────────────────────────────
  return(
    <View style={{width:SW, height:SH, backgroundColor:theme.bg, overflow:'hidden', position:'relative'}}>
        ...
        <Animated.View style={{
          ...
          <View style={{position:'absolute',left:0,top:0,width:WORLD_W*ZOOM,height:WORLD_H*ZOOM}}>
            ...
            {cf.freezeGun&&!cf.freezeGun.taken&&(
              <View style={{
                position:'absolute',
                left:(cf.freezeGun.x-18)*ZOOM,top:(cf.freezeGun.y-18)*ZOOM,
                width:36*ZOOM,height:36*ZOOM,borderRadius:18*ZOOM,
                backgroundColor:nearFreeze?'rgba(100,220,255,0.55)':'rgba(40,120,200,0.30)',
                borderWidth:nearFreeze?2:1.5,borderColor:nearFreeze?'#aaeeff':'#5599bb',
                justifyContent:'center',alignItems:'center',
                shadowColor:'#88ddff',shadowRadius:8*ZOOM,shadowOpacity:nearFreeze?0.9:0.4,
              }}>
                <Text style={{fontSize:18*ZOOM}}>❄</Text>
            ...
            {cf.pushPickups&&cf.pushPickups.map((pp,i)=>{
              ...
              return(
                <View key={`push${i}`} style={{
                  position:'absolute',
                  left:(pp.x-18)*ZOOM,top:(pp.y-18)*ZOOM,
                  width:36*ZOOM,height:36*ZOOM,borderRadius:18*ZOOM,
                  backgroundColor:nearPP?'rgba(255,100,40,0.65)':'rgba(200,60,10,0.32)',
                  borderWidth:nearPP?2.5:1.5,borderColor:nearPP?'#ff7744':'#cc4422',
                  justifyContent:'center',alignItems:'center',
                  shadowColor:'#ff5500',shadowRadius:nearPP?12*ZOOM:5*ZOOM,shadowOpacity:0.85,
                }}>
                  <Text style={{fontSize:18*ZOOM}}>💥</Text>
            ...
            {/* ── DARK + MIC OVERLAY ─────────────────────────────────────── */}
            {darkMic.phase==='active'&&(
              <View pointerEvents="none" style={{position:'absolute',left:0,top:0,width:WORLD_W*ZOOM,height:WORLD_H*ZOOM,backgroundColor:'rgba(0,0,0,0.38)'}}/>
            )}
      ...
      <View style={{
        height:CONTROLS_H,
        paddingBottom:SAFE_BOT,paddingTop:6,paddingHorizontal:10,
        flexDirection:'row',alignItems:'center',gap:8,
        ...
        <View style={{flexShrink:0, gap:2}}>
          <View style={{flexDirection:'row',gap:2}}>
            <View style={{width:ACT_H,height:ACT_H}}/>
            <TouchableOpacity disabled={elevatorBusy||pauseOpen}
              style={{width:ACT_H,height:ACT_H,borderRadius:8,backgroundColor:theme.edge+'22',borderWidth:1.5,borderColor:theme.edge+'88',alignItems:'center',justifyContent:'center',opacity:elevatorBusy?0.3:1}}
              onPressIn={()=>{moveRef.current.up=true;registerTap('up');}} onPressOut={()=>moveRef.current.up=false}>
              <Text style={{color:'white',fontWeight:'900',fontSize:13}}>↑</Text>
            </TouchableOpacity>
            <View style={{width:ACT_H,height:ACT_H}}/>
          </View>
          <View style={{flexDirection:'row',gap:2}}>
            ...
            <View style={{width:ACT_H,height:ACT_H,borderRadius:8,backgroundColor:theme.edge+'0a'}}/>
            <TouchableOpacity disabled={elevatorBusy||pauseOpen}
              style={{width:ACT_H,height:ACT_H,borderRadius:8,backgroundColor:theme.edge+'22',borderWidth:1.5,borderColor:theme.edge+'88',alignItems:'center',justifyContent:'center',opacity:elevatorBusy?0.3:1}}
        ...
        {/* ACTION BUTTONS — all 5 in one single row */}
        <TouchableOpacity
          disabled={pauseOpen||elevatorBusy||freezeActive}
          style={{paddingHorizontal:14,height:ACT_H,borderRadius:8,
            ...
            borderWidth:1.5,borderColor:inv.freezeCharges>0&&!freezeActive?'#66ccff':freezeActive?'rgba(100,200,255,0.60)':'rgba(100,180,220,0.20)',
            alignItems:'center',justifyContent:'center',
            opacity:(!pauseOpen&&!elevatorBusy&&!freezeActive)?1:0.45}}
          onPress={activateFreeze}>
          <Text style={{color:inv.freezeCharges>0&&!freezeActive?'#88ddff':freezeActive?'#55ccff':'rgba(100,180,220,0.40)',fontWeight:'900',fontSize:10,fontFamily:'monospace',letterSpacing:0.5}}>
            ● FREEZE {inv.freezeCharges}
          </Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity
          ...
          style={{paddingHorizontal:14,height:ACT_H,borderRadius:8,
            ...
            borderWidth:1.5,borderColor:ghostIsClose&&inv.pushCharges>0?'#ff6633':'rgba(255,100,50,0.20)',
            alignItems:'center',justifyContent:'center',
            opacity:(!pauseOpen&&!elevatorBusy&&!freezeActive)?1:0.38}}
          onPress={pushGhost}>
          <Text style={{color:ghostIsClose&&inv.pushCharges>0?'#ff8855':'rgba(255,120,80,0.38)',fontWeight:'900',fontSize:10,fontFamily:'monospace',letterSpacing:0.5}}>
            ● PUSH {inv.pushCharges}
          </Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity
          ...
          style={{paddingHorizontal:14,height:ACT_H,borderRadius:8,
            ...
            alignItems:'center',justifyContent:'center',opacity:(inv.shieldCharges>0&&!pauseOpen)?1:0.38}}
          onPress={toggleShield}>
          <Text style={{color:inv.shieldCharges>0?theme.accent:'rgba(255,255,255,0.28)',fontWeight:'900',fontSize:10,fontFamily:'monospace',letterSpacing:0.5}}>
            {inv.shieldArmed?'SHIELD ON':'SHIELD OFF'} {inv.shieldCharges}
          </Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity
          ...
          style={{paddingHorizontal:14,height:ACT_H,borderRadius:8,
            ...
            alignItems:'center',justifyContent:'center',opacity:(canCapture&&!pauseOpen&&!elevatorBusy)?1:0.35}}
          onPressIn={()=>onCaptureDown(null)} onPressOut={onCaptureUp}>
          <Text style={{color:canCapture?'white':'rgba(255,255,255,0.35)',fontWeight:'900',fontSize:10,fontFamily:'monospace',letterSpacing:0.5}}>
            {captureLabel}
          </Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity
          ...
          style={{paddingHorizontal:14,height:ACT_H,borderRadius:8,
            ...
            alignItems:'center',justifyContent:'center',
            opacity:(inElevator&&!elevatorBusy&&!pauseOpen&&!panic.active)?1:0.38}}
          onPress={elevatorSequence}>
          <Text style={{
            color:inElevator&&allKeys?'#80ffb0':inElevator?'#d6c8ff':'rgba(255,255,255,0.32)',
            fontWeight:'900',fontSize:10,fontFamily:'monospace',letterSpacing:0.5}}>
            {inElevator&&allKeys?'OPEN DOOR':'ELEVATOR'}
          </Text>
        </TouchableOpacity>
...
function AchievementsScreen({ navigation }){
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      try{
        const lr = await storage.get('LASTFLOOR_LASTRESULT_V1');
...
function SettingsScreen({ navigation }){
  ...
  const save = async(nextMic)=>{
    setMic(nextMic);
    try{
      const raw = await storage.get(PROFILE_KEY);
      const j = raw ? JSON.parse(raw) : {};
      await storage.set(PROFILE_KEY, JSON.stringify({...j, settings:{...(j?.settings||{}), micEnabled:nextMic}}));
    }catch(e){}
  };

  const Toggle = ({label, desc, value, onTrue, onFalse, colorOn='#5ecfa0', colorOff='#ff5566'})=>(
    <View style={{padding:18,borderRadius:16,backgroundColor:'rgba(255,255,255,0.025)',
      borderWidth:1,borderColor:'rgba(255,255,255,0.08)',marginBottom:12}}>
      <Text style={{color:'#e8dcc8',fontFamily:'monospace',fontWeight:'900',fontSize:12,letterSpacing:2,marginBottom:4}}>{label}</Text>
      <Text style={{color:'rgba(255,255,255,0.38)',fontFamily:'monospace',fontSize:10,lineHeight:16,marginBottom:14}}>{desc}</Text>
      <View style={{flexDirection:'row',gap:10}}>
        <TouchableOpacity onPress={onTrue} activeOpacity={0.85} style={{
          flex:1,paddingVertical:13,borderRadius:12,alignItems:'center',
          borderWidth:1.5,
          borderColor: value ? colorOn : 'rgba(255,255,255,0.1)',
          backgroundColor: value ? colorOn+'18' : 'rgba(255,255,255,0.02)',
        }}>
          <Text style={{color: value ? colorOn : 'rgba(255,255,255,0.35)',
            fontFamily:'monospace',fontWeight:'900',fontSize:11,letterSpacing:3}}>ENABLED</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={onFalse} activeOpacity={0.85} style={{
          flex:1,paddingVertical:13,borderRadius:12,alignItems:'center',
          borderWidth:1.5,
          borderColor: !value ? colorOff : 'rgba(255,255,255,0.1)',
          backgroundColor: !value ? colorOff+'18' : 'rgba(255,255,255,0.02)',
        }}>
          <Text style={{color: !value ? colorOff : 'rgba(255,255,255,0.35)',
            fontFamily:'monospace',fontWeight:'900',fontSize:11,letterSpacing:3}}>DISABLED</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
      ...
      <Animated.View style={{flex:1,opacity:fadeAnim}}>
        <ScrollView contentContainerStyle={{padding:20,paddingBottom:40}} showsVerticalScrollIndicator={false}>
          ...
          <Toggle
            ...
            desc={"In Dark Mode, loud sounds attract nearby ghosts.Enable to use your real microphone for this effect."}
            value={mic}
            onTrue={()=>save(true)}
            onFalse={()=>save(false)}
          />

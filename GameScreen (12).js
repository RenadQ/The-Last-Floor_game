...
import { useAppState } from '../context/GameContext';
import { storage, clearAllGameStorage, PROFILE_KEY, CHAR_KEY, STATS_KEY, COINS_KEY, LOCATION_KEY, LASTRESULT_KEY, ALL_STORAGE_KEYS, CONTINUE_COST_BASE, defaultProfile, defaultStats, _memStore } from '../utils/storage';
import { ELEVATOR_QUESTION_COINS, ELEVATOR_QUESTIONS } from '../utils/questions';
import { getRandomRiddle } from '../services/riddleService';
import {
  View, Text, TextInput, TouchableOpacity, StyleSheet, Animated, Modal,
  Dimensions, StatusBar, Vibration, ScrollView,
  Platform, Alert, Easing, Image, useWindowDimensions,
} from 'react-native';

// ─── DEBUG LOGGING ─────────────────────────────────────────────────────────
console.log('🟦 [GameScreen] Module loaded on platform:', Platform.OS);
// ───────────────────────────────────────────────────────────────────────────

// Safe import for react-native-svg - iOS may not support it properly
let Svg, Path, Circle, Ellipse, Rect, G, Defs, Mask, RadialGradient, Stop, Line;
let SVG_AVAILABLE = false;
try {
  ...
  Line = svgModule.Line;
  SVG_AVAILABLE = true;
  console.log('✅ react-native-svg loaded successfully');
} catch(err) {
  console.warn('⚠️ react-native-svg not available, using fallback components:', err.message);
  // Create dummy components that return null
  Svg = () => null;
  Path = () => null;
  Circle = () => null;
  Ellipse = () => null;
...
try {
  ...
  const candidate = avModule.Video;
  console.log('🟦 [expo-av] Video type:', typeof candidate, '| keys:', Object.keys(avModule).join(','));
  if (typeof candidate === 'function') {
    Video = candidate;
    console.log('✅ [expo-av] Video loaded as function — OK');
  } else {
    Video = null;
    console.warn('⚠️ [expo-av] Video is NOT a function (type:', typeof candidate, ') — disabling to prevent crash');
  }
} catch (e) {
  Audio = null; Video = null; ResizeMode = null;
  console.warn('⚠️ [expo-av] import failed:', e.message);
}

// Safe import for expo-screen-orientation
...
function HubTabs({ onStartGame, profile, setProfile, openSettings, openAchievements, charData, onEditCharacter }) {
  ...
  useEffect(()=>{
    ...
    const doFlicker = () => {
      ...
      Animated.sequence(seqItems).start(({finished})=>{ if(finished && alive) doFlicker(); });
    };
    doFlicker();
    // Spider web breathing pulse
    const webLoop = Animated.loop(Animated.sequence([
    ...
    return ()=>{
      alive = false;
      webLoop.stop();
      globalFlicker.stopAnimation();
      webPulse.stopAnimation();
    };
...
// ── API 1: ZenQuotes — moody quote shown on HomeScreen ───────────────────────
// Endpoint: GET https://zenquotes.io/api/random
// No auth required. Fallback to hardcoded horror quote on error.
const ZENQUOTES_URL = 'https://zenquotes.io/api/random';
const FALLBACK_QUOTES = [
  { q: 'The oldest and strongest emotion of mankind is fear.', a: 'H.P. Lovecraft' },
  { q: 'No live organism can continue for long to exist sanely under conditions of absolute reality.', a: 'Shirley Jackson' },
  { q: 'Monsters are real, and ghosts are real too. They live inside us.', a: 'Stephen King' },
];

function useZenQuote() {
  const [quote, setQuote]     = useState(null);   // { q, a }
  ...
  useEffect(()=>{
    ...
    fetch(ZENQUOTES_URL, { method:'GET', headers:{ 'Accept':'application/json' } })
      .then(r=>{ if(!r.ok) throw new Error('HTTP '+r.status); return r.json(); })
      .then(data=>{
        if(!alive) return;
        // ZenQuotes returns array: [{ q, a, h }]
      ...
      .catch(()=>{
        if(!alive) return;
        // Fallback: pick a random local horror quote
...
function HomeScreen({ profile, charData, onPlay, lh=300, lw=600 }) {
  ...
  // ── API 1: ZenQuotes fetch ────────────────────────────────────────────────
  const { quote: zenQuote, loading: zenLoading, error: zenError } = useZenQuote();
  const quoteFade = useRef(new Animated.Value(0)).current;
  useEffect(()=>{
    if(!zenLoading){
      Animated.timing(quoteFade,{toValue:1,duration:700,useNativeDriver:true}).start();
    }
  },[zenLoading]);
  ...
  // ── NEW: title glow pulse + web breathe ──────────────────────────────────
  const titleGlow   = useRef(new Animated.Value(0)).current;
  const webBreath   = useRef(new Animated.Value(0)).current;
  ...
  useEffect(()=>{
    ...
    const flicker = ()=>{
      ...
      activeAnims.push(seq);
      seq.start(({finished})=>{ if(finished && alive) flicker(); });
    };
    flicker();
    ...
    const drip = (anim, delay, duration)=>{
      const run = ()=>{
        ...
        activeAnims.push(seq);
        seq.start(({finished})=>{ if(finished && alive) run(); });
      };
      run();
    };
    ...
    // Fog pulse
    const fogLoop = Animated.loop(Animated.sequence([
    ...
    // Title glow — irregular pulse for horror feel
    const glowLoop = () => {
      if(!alive) return;
      const seq = Animated.sequence([
        Animated.timing(titleGlow,{toValue:1,duration:800+Math.random()*600,useNativeDriver:true}),
        Animated.timing(titleGlow,{toValue:0.35,duration:400+Math.random()*400,useNativeDriver:true}),
        Animated.timing(titleGlow,{toValue:1,duration:600,useNativeDriver:true}),
        Animated.delay(1200+Math.random()*2000),
        Animated.timing(titleGlow,{toValue:0.1,duration:80,useNativeDriver:true}),
        Animated.timing(titleGlow,{toValue:1,duration:120,useNativeDriver:true}),
        Animated.delay(500),
      ]);
      activeAnims.push(seq);
      seq.start(({finished})=>{ if(finished && alive) glowLoop(); });
    };
    glowLoop();

    // Spider web corner breath
    const webLoop = Animated.loop(Animated.sequence([
      Animated.timing(webBreath,{toValue:1,duration:4000,useNativeDriver:true}),
      Animated.timing(webBreath,{toValue:0,duration:4000,useNativeDriver:true}),
    ]));
    webLoop.start();
    ...
    return ()=>{
      ...
      pulseLoop.stop();
      webLoop.stop();
      activeAnims.forEach(a=>{ try{ a.stop(); }catch(e){} });
      flickerAnim.stopAnimation();
      bloodDrip1.stopAnimation();
      bloodDrip2.stopAnimation();
      bloodDrip3.stopAnimation();
      fogAnim.stopAnimation();
      pulseAnim.stopAnimation();
      titleGlow.stopAnimation();
      webBreath.stopAnimation();
    };
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[lh]);
  ...
  return (
    <View style={{flex:1, flexDirection:'column', overflow:'hidden'}}>
      ...
      <View style={{position:'absolute',inset:0}} pointerEvents="none">
        ...
        {/* ── SPIDER WEBS — four corners ───────────────────────────────── */}
        {[
          {corner:'TL', x:0,   y:0,   flip:false, flipV:false},
          {corner:'TR', x:null,y:0,   flip:true,  flipV:false},
          {corner:'BL', x:0,   y:null,flip:false, flipV:true},
          {corner:'BR', x:null,y:null,flip:true,  flipV:true},
        ].map(({corner,x,y,flip,flipV})=>{
          const sz = Math.round(Math.min(lw,lh)*0.22);
          const spokes=7; const rings=[0.25,0.45,0.65,0.85,1.0];
          const paths=[];
          for(let i=0;i<spokes;i++){
            const a=(i/spokes)*Math.PI*0.5;
            const ex=Math.cos(a)*sz*(flip?-1:1);
            const ey=Math.sin(a)*sz*(flipV?-1:1);
            paths.push(`M0,0 L${ex},${ey}`);
          }
          rings.forEach(r=>{
            let d=`M${Math.cos(0)*sz*r*(flip?-1:1)},${Math.sin(0)*sz*r*(flipV?-1:1)}`;
            for(let i=1;i<=spokes;i++){
              const a=(i/spokes)*Math.PI*0.5;
              d+=` L${Math.cos(a)*sz*r*(flip?-1:1)},${Math.sin(a)*sz*r*(flipV?-1:1)}`;
            }
            paths.push(d);
          });
          return(
            <Animated.View key={corner} style={{
              position:'absolute',
              left:x!==null?x:undefined, right:x===null?0:undefined,
              top:y!==null?y:undefined,  bottom:y===null?0:undefined,
              opacity:webBreath.interpolate({inputRange:[0,1],outputRange:[0.40,0.80]}),
            }}>
              <Svg width={sz+4} height={sz+4}>
                <G>
                  {paths.map((d,pi)=>(
                    <Path key={pi} d={d} stroke="rgba(180,180,255,0.18)" strokeWidth={0.9} fill="none"/>
                  ))}
                </G>
              </Svg>
            </Animated.View>
      ...
      <Animated.View style={{flex:1,flexDirection:'column',opacity:flickerAnim}}>
        ...
        <View style={{
          ...
          <Animated.Text style={{
            ...
            textAlign:'center',
            // Glow achieved via textShadowRadius driven by titleGlow anim
            textShadowColor: 'rgba(201,164,76,0.95)',
            textShadowRadius: titleGlow.interpolate({inputRange:[0,1],outputRange:[4,28]}),
            opacity: titleGlow.interpolate({inputRange:[0,1],outputRange:[0.70,1.0]}),
          }}>THE LAST FLOOR</Animated.Text>
          {/* Team names */}
          <Text style={{
        ...
        <View style={{flex:1, flexDirection:'row', paddingHorizontal:10, paddingVertical:6, gap:10}}>
          ...
          <View style={{flex:1, justifyContent:'center', gap: isSmall?5:8}}>
            ...
            {/* ── API 1: ZenQuotes — live horror quote (replaces static ghost warning) ── */}
            <Animated.View style={{
              flexDirection:'row', alignItems:'flex-start', gap:8,
              borderRadius:11, paddingVertical:8, paddingHorizontal:10,
              backgroundColor:'rgba(255,20,20,0.07)',
              borderWidth:1, borderColor:'rgba(255,60,60,0.18)',
              opacity: quoteFade, minHeight:44,
            }}>
              {zenLoading ? (
                <View style={{flex:1,flexDirection:'row',alignItems:'center',gap:6}}>
                  <Text style={{fontSize:12}}>👻</Text>
                  <Text style={{color:'rgba(255,100,100,0.40)',fontFamily:'monospace',fontSize:isSmall?7:8,fontStyle:'italic'}}>
                    Fetching a dark omen…
                  </Text>
                </View>
              ) : (
                <View style={{flex:1}}>
                  <Text style={{color:'rgba(255,100,100,0.60)',fontFamily:'monospace',
                    fontSize:isSmall?7:8,letterSpacing:1,flex:1,lineHeight:13}}>
                    {zenError ? '👻 ' : '🩸 '}
                    "{zenQuote?.q}"
                  </Text>
                  <Text style={{color:'rgba(255,80,80,0.35)',fontFamily:'monospace',
                    fontSize:6,letterSpacing:1,marginTop:3,alignSelf:'flex-end'}}>
                    — {zenQuote?.a}{zenError?' (offline)':''}
                  </Text>
                </View>
              )}
            </Animated.View>
...
function GhostRiddleScreen({ onDone }) {
  const { width: W, height: H } = useWindowDimensions();
  const LW = Math.max(W, H), LH = Math.min(W, H);
  const isSmall = LH < 340;
  ...
  const [apiSource, setApiSource] = useState(null); // 'api' | 'fallback' | null

  const fadeIn = useRef(new Animated.Value(0)).current;
  const cardPop = useRef(new Animated.Value(0.96)).current;
  const glowPulse = useRef(new Animated.Value(0)).current;

  useEffect(() => {
    Animated.parallel([
      Animated.timing(fadeIn, { toValue: 1, duration: 500, useNativeDriver: true }),
      Animated.spring(cardPop, { toValue: 1, friction: 7, tension: 70, useNativeDriver: true }),
    ]).start();
    ...
    const cleanupFetch = fetchRiddle();

    return () => {
      cleanupFetch?.();
      glowLoop.stop();
      fadeIn.stopAnimation();
      cardPop.stopAnimation();
  ...
  /**
   * fetchRiddle()
   * Loads a riddle from the external Riddles API (riddleService.js).
   * Falls back to local ELEVATOR_QUESTIONS when offline.
   * Returns a cleanup function that sets an 'alive' flag to prevent
  ...
  const fetchRiddle = () => {
    ...
    getRandomRiddle().then((riddle) => {
      if (!alive) return;

      if (riddle.source === 'api' || !riddle.options) {
        // API riddle or open-answer: build fake options from local list for
        // multiple-choice UI, but use API question text & answer.
        const localPool = ELEVATOR_QUESTIONS.filter(
          q => q.correctAnswer !== riddle.answer
        );
        const distractors = shuffleArray(localPool)
          .slice(0, 3)
          .map(q => q.correctAnswer);
        const builtChoices = shuffleArray([riddle.answer, ...distractors]);
        setQuestionText(riddle.question);
        setCorrectAnswer(riddle.answer);
        setChoices(builtChoices);
      } else {
        // Local fallback riddle — already has multiple-choice options
        setQuestionText(riddle.question);
        setCorrectAnswer(riddle.answer);
        setChoices(shuffleArray(riddle.options));
      }

      setApiSource(riddle.source);
      setStatus('ready');
    }).catch(() => {
      // Should never reach here (riddleService catches internally), but be safe
      if (!alive) return;
      const fallback = ELEVATOR_QUESTIONS[Math.floor(Math.random() * ELEVATOR_QUESTIONS.length)];
      setQuestionText(fallback.question);
      setCorrectAnswer(fallback.correctAnswer);
      setChoices(shuffleArray([...fallback.options]));
  ...
  const chooseAnswer = (choice) => {
    ...
    setSelected(choice);
    setWonCoins(correct ? ELEVATOR_QUESTION_COINS : 0);
    setPhase('result');
    setTimeout(() => onDone(correct ? ELEVATOR_QUESTION_COINS : 0), 1400);
  };

  return (
    <ScrollView
      style={{ width: LW, height: LH, backgroundColor: '#04020e' }}
      contentContainerStyle={{
        minHeight: LH,
        alignItems: 'center',
        justifyContent: 'center',
        paddingHorizontal: Math.round(LW * 0.05),
        paddingVertical: 16,
      }}
      showsVerticalScrollIndicator={false}
    >
      <Animated.View style={{ width: '100%', maxWidth: 620, opacity: fadeIn, transform: [{ scale: cardPop }] }}>
        <StatusBar hidden />

        <Animated.View
          style={{
            position: 'absolute', top: 0, left: 0, right: 0, bottom: 0,
            backgroundColor: 'rgba(60,0,100,0.08)',
            opacity: glowPulse,
          }}
          pointerEvents="none"
        />
        ...
        <View style={{
          ...
          marginBottom: isSmall ? 10 : 18,
        }}>
          <View>
            <Text style={{ color: 'rgba(180,100,255,0.40)', fontFamily: 'monospace', fontSize: 7, letterSpacing: 5, marginBottom: 2 }}>
              THE LAST FLOOR · ELEVATOR
            </Text>
            <Text style={{ color: '#d6c8ff', fontFamily: 'monospace', fontWeight: '900', fontSize: isSmall ? 14 : 18, letterSpacing: 4 }}>
              CHOOSE THE ANSWER
            </Text>
          </View>
          {phase !== 'result' && (
            <View style={{ flexDirection: 'row', gap: 8 }}>
              {/* Retry: fetch a new riddle (works for both API and offline) */}
              <TouchableOpacity onPress={() => { const cleanup = fetchRiddle(); return cleanup; }}
                activeOpacity={0.75}
                style={{ paddingVertical: 8, paddingHorizontal: 12, borderRadius: 9,
                  backgroundColor: 'rgba(180,100,255,0.08)', borderWidth: 1,
                  borderColor: 'rgba(180,100,255,0.25)' }}>
                <Text style={{ color: 'rgba(180,100,255,0.70)', fontFamily: 'monospace',
                  fontSize: 9, letterSpacing: 2 }}>↺ NEW</Text>
              </TouchableOpacity>
              {/* Skip */}
              <TouchableOpacity onPress={() => onDone(0)} activeOpacity={0.75}
                style={{ paddingVertical: 8, paddingHorizontal: 14, borderRadius: 9,
                  backgroundColor: 'rgba(255,255,255,0.03)', borderWidth: 1,
                  borderColor: 'rgba(255,255,255,0.10)' }}>
                <Text style={{ color: 'rgba(255,255,255,0.28)', fontFamily: 'monospace',
                  fontSize: 9, letterSpacing: 2 }}>SKIP</Text>
                <Text style={{ color: 'rgba(255,255,255,0.15)', fontFamily: 'monospace',
                  fontSize: 7, marginTop: 1, textAlign: 'center' }}>no coins</Text>
              </TouchableOpacity>
            </View>
          )}
        </View>
        ...
        {status === 'loading' && (
          <View style={{ alignItems: 'center', gap: 12, paddingVertical: 28 }}>
            ...
            <Text style={{ color: 'rgba(180,140,255,0.60)', fontFamily: 'monospace', fontSize: 11, letterSpacing: 2 }}>
              The ghost is preparing a question…
            </Text>
          </View>
        )}

        {status === 'ready' && (
          <View style={{ width: '100%', maxWidth: 600, gap: isSmall ? 10 : 14 }}>
            <View style={{
              padding: isSmall ? 14 : 18, borderRadius: 16,
              backgroundColor: 'rgba(80,0,160,0.14)', borderWidth: 1.5, borderColor: 'rgba(180,100,255,0.30)',
              shadowColor: '#8800ff', shadowRadius: 14, shadowOpacity: 0.25,
            }}>
              <Text style={{ color: 'rgba(180,100,255,0.55)', fontFamily: 'monospace', fontSize: 7, letterSpacing: 4, marginBottom: 8 }}>
                QUESTION · MULTIPLE CHOICE
              </Text>
              <Text style={{
                color: '#e8dce8', fontFamily: 'monospace', fontWeight: '900',
                fontSize: Math.round(Math.max(12, Math.min(15, LH * 0.032))),
                lineHeight: Math.round(Math.max(18, Math.min(22, LH * 0.048))),
              }}>
                {questionText}
              </Text>
            </View>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6,
              paddingVertical: 5, paddingHorizontal: 12, borderRadius: 16, alignSelf: 'center',
              backgroundColor: 'rgba(201,164,76,0.10)', borderWidth: 1, borderColor: 'rgba(201,164,76,0.25)' }}>
              <Text style={{ fontSize: 11 }}>🪙</Text>
              <Text style={{ color: '#c9a44c', fontFamily: 'monospace', fontSize: 9, letterSpacing: 1 }}>
                Correct answer → +{ELEVATOR_QUESTION_COINS} coins
              </Text>
            </View>

            {apiSource === 'fallback' && (
              <View style={{ alignSelf: 'center', paddingHorizontal: 10, paddingVertical: 4,
                borderRadius: 8, backgroundColor: 'rgba(255,200,80,0.10)',
                borderWidth: 1, borderColor: 'rgba(255,200,80,0.30)', marginTop: -4 }}>
                <Text style={{ color: 'rgba(255,200,80,0.70)', fontFamily: 'monospace',
                  fontSize: 8, letterSpacing: 2 }}>
                  📡 OFFLINE — LOCAL QUESTION
                </Text>
              </View>
            )}
            ...
            <View style={{ gap: 9 }}>
              {choices.map((choice, idx) => {
                ...
                const bg = !showResult
                  ...
                  : isCorrect
                    ? 'rgba(80,220,120,0.14)'
                    : isSelected
                      ? 'rgba(255,60,60,0.10)'
                      : 'rgba(255,255,255,0.03)';
                const border = !showResult
                  ? 'rgba(180,100,255,0.22)'
                  : isCorrect
                    ? 'rgba(80,220,120,0.50)'
                    : isSelected
                      ? 'rgba(255,80,80,0.35)'
                ...
                return (
                  ...
                  >
                    <Text style={{ color: textColor, fontFamily: 'monospace', fontWeight: '900', fontSize: 11, letterSpacing: 1.5 }}>
            ...
            {phase === 'result' && (
              <View style={{
                alignItems: 'center', paddingVertical: 12, borderRadius: 13,
                backgroundColor: wonCoins > 0 ? 'rgba(201,164,76,0.12)' : 'rgba(255,255,255,0.04)',
                borderWidth: 1.5,
                borderColor: wonCoins > 0 ? 'rgba(201,164,76,0.40)' : 'rgba(255,255,255,0.10)',
              }}>
                <Text style={{ fontSize: 18 }}>{wonCoins > 0 ? '🪙' : '👻'}</Text>
                <Text style={{ color: wonCoins > 0 ? '#c9a44c' : 'rgba(255,255,255,0.75)', fontFamily: 'monospace', fontWeight: '900', fontSize: 13, letterSpacing: 2, marginTop: 4 }}>
                  {wonCoins > 0 ? `+${ELEVATOR_QUESTION_COINS} COINS EARNED` : 'WRONG ANSWER'}
                </Text>
                <Text style={{ color: 'rgba(201,164,76,0.50)', fontFamily: 'monospace', fontSize: 9, marginTop: 2 }}>
                  Entering the floor…
                </Text>
              </View>
            )}
...
function RulesScreen() {
  ...
  return (
    <ScrollView style={{flex:1,backgroundColor:'#030008'}}
      ...
      <Section label="DIFFICULTY TIERS" idx={2}/>
      <FadeIn idx={2}>
      ...
      <Section label="SILENCE WARNING" idx={3}/>
      <FadeIn idx={3}>
      ...
      <Section label="CONTROLS" idx={4}/>
      <Rule sym="KEY" symColor="#c9a44c" idx={4}
      ...
      <Rule sym="UP" symColor="#a090ff" idx={5}
...
function IntroVideoScreen({ onDone }) {
  ...
  return (
    <Animated.View style={{
      ...
      {Platform.OS !== 'web' && typeof Video === 'function' && videoSource && (
        <Video
          source={videoSource}
      ...
      {!videoSource && (
        <View style={{ width:VW, height:VH, justifyContent:'center', alignItems:'center', backgroundColor:'#000' }}>
          <Image
            ...
            style={{ width: Math.min(VW * 0.72, 420), height: Math.min(VH * 0.72, 420), resizeMode:'contain' }}
          />
          <Text style={{ color:'rgba(255,255,255,0.55)', fontFamily:'monospace', fontSize:11, letterSpacing:3, marginTop:18 }}>
            INTRO VIDEO NOT FOUND, USING SPLASH FALLBACK
          </Text>
        </View>
      )}
...
function TypingIntro({ name, onDone }) {
  ...
  useEffect(()=>{
    ...
    // Pulse the READY? word once it's visible — store ref so we can stop on unmount
    let pulseLoopAnim = null;
    const startPulse = () => {
      if(!alive) return;
      pulseLoopAnim = Animated.loop(Animated.sequence([
        Animated.timing(pulseAnim,{toValue:1.08,duration:700,useNativeDriver:true}),
        Animated.timing(pulseAnim,{toValue:1.0, duration:700,useNativeDriver:true}),
      ]));
      pulseLoopAnim.start();
    };
    const pt = setTimeout(startPulse, 2800);
    return ()=>{
      alive=false;
      clearTimeout(pt);
      if(pulseLoopAnim) pulseLoopAnim.stop();
      pulseAnim.stopAnimation();
...
const MIC_ENABLED_DEFAULT = false;
...
function buildFloors(){
  ...
  //
  //  di=0  EASY   : small maze, 3 ghosts, generous items, key nearby
  //  di=1  MEDIUM : medium maze, 4 ghosts, moderate items
  //  di=2  HARD   : large maze, 5 ghosts with bigger zones
  //  di=3  BRUTAL : huge maze, 6 FAST ghosts, very large zones, scarce items
  //
  const configs=[
    // di=0 EASY - زيادة الوحوش من 2 إلى 3
    {cols:10,rows:5, cellW:300,cellH:260, ox:32,oy:32, wallT:30,
     numGhosts:3, ghostKinds:['NORMAL','NORMAL','NORMAL'], zoneBase:320, zonePerDi:0,
     numPush:2, hasBandage:true, hasFreeze:true, hasShield:false, hasSpeed:true, hasKill:false,
     minKeyDistMult:0.18, diffLabel:'EASY'},
    // di=1 MEDIUM - زيادة الوحوش من 3 إلى 4
    {cols:14,rows:7, cellW:220,cellH:196, ox:32,oy:32, wallT:26,
     numGhosts:4, ghostKinds:['NORMAL','NORMAL','NORMAL','NORMAL'], zoneBase:380, zonePerDi:30,
     numPush:2, hasBandage:true, hasFreeze:true, hasShield:true, hasSpeed:false, hasKill:false,
     minKeyDistMult:0.22, diffLabel:'MEDIUM'},
    // di=2 HARD - زيادة الوحوش من 4 إلى 5
    {cols:18,rows:8, cellW:174,cellH:168, ox:32,oy:32, wallT:24,
     numGhosts:5, ghostKinds:['NORMAL','NORMAL','NORMAL','FAST','NORMAL'], zoneBase:440, zonePerDi:50,
     numPush:1, hasBandage:true, hasFreeze:true, hasShield:true, hasSpeed:false, hasKill:true,
     minKeyDistMult:0.26, diffLabel:'HARD'},
    // di=3 BRUTAL - زيادة الوحوش من 4 إلى 6
    {cols:22,rows:10, cellW:144,cellH:138, ox:32,oy:32, wallT:22,
     numGhosts:6, ghostKinds:['FAST','NORMAL','FAST','NORMAL','FAST','FAST'], zoneBase:500, zonePerDi:60,
    ...
    <View pointerEvents="auto" style={{position:'absolute',left:0,right:0,top:0,bottom:0,alignItems:'center',justifyContent:'center'}}>
      ...
      <Animated.View style={{
        ...
        <Text style={{color:'rgba(255,255,255,0.40)',fontFamily:'monospace',fontSize:10,lineHeight:16,marginBottom:14}}>
          {confirmExit ? 'Press EXIT again to fully reset this run and return to the main hub.' : 'Leaving will NOT save your progress.'}
        </Text>

        <View style={{flexDirection:'row',gap:10}}>
          <TouchableOpacity
            onPress={()=>{
              if(confirmExit){ onExit?.(); return; }
              setConfirmExit(true);
            }}
            activeOpacity={0.85}
            ...
            <Text style={{color:'rgba(255,160,180,0.95)',fontFamily:'monospace',fontWeight:'900',letterSpacing:2}}>{confirmExit ? 'CONFIRM EXIT' : 'EXIT'}</Text>
          </TouchableOpacity>
    ...
    const t = setTimeout(()=>{
      ...
      Animated.parallel([
        ...
        const loopScan=()=>{
          ...
          ]).start(({finished})=>{ if(finished && aliveRef.current) loopScan(); });
        };
        loopScan();
...
function TitleScreen({onStart}){
  ...
  useEffect(()=>{
    ...
    const gyLoop=()=>{
      if(!alive) return;
      Animated.sequence([
        Animated.timing(gy,{toValue:-14,duration:900,useNativeDriver:true}),
        Animated.timing(gy,{toValue:0,duration:800,useNativeDriver:true}),
        Animated.delay(300),
      ]).start(({finished})=>{ if(finished && alive) gyLoop(); });
    };
    gyLoop();
    return ()=>{ alive=false; gy.stopAnimation(); fade.stopAnimation(); };
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[]);
  return(
...
function DeadScreen({charData,onMenu,floorNum=1,totalCoins=0,continueCost=15,onContinuePaid}){
  ...
  return(
    <Animated.View style={{width:LW,height:LH,backgroundColor:'#060004',opacity:fade}}>
      <StatusBar hidden/>
      <View style={{position:'absolute',top:0,left:0,right:0,height:3,backgroundColor:'#aa0015'}}/>
      <ScrollView
        contentContainerStyle={{flexDirection:'row',minHeight:LH}}
        horizontal={false}
        showsVerticalScrollIndicator={false}
        bounces={false}
      >
      <View style={{width:LW,flexDirection:'row',minHeight:LH}}>
...
function WinScreen({charData,onPlay,onMenu,durMs=0,bestEscapeMs=0,brokeRecord=false,escapes=0}){
  ...
  return(
    <Animated.View style={{width:LW,height:LH,backgroundColor:'#020b06',opacity:fade}}>
      <StatusBar hidden/>
      <ScrollView
        contentContainerStyle={{flexDirection:'row',minHeight:LH}}
        showsVerticalScrollIndicator={false}
...
function GameCore({ navigation }){
  console.log('🟦 [GameCore] Rendered/mounted');
  ...
  const setScreenWithLog = useCallback((s) => {
    console.log(`🔄 [GameCore] screen: "${screen}" → "${s}"`);
    setScreen(s);
  }, [screen]);
  ...
  // MOVE_BTN_SIZE: larger size for movement buttons (up, down, left, right) - 60% bigger
  const MOVE_BTN_SIZE = Math.round(ACT_H * 1.6);
  const SAFE_BOT   = Platform.OS === 'ios' ? 10 : 2;
  // Single row of controls — using larger MOVE_BTN_SIZE for movement buttons
  const CONTROLS_H = MOVE_BTN_SIZE + SAFE_BOT + 14;
  // ZOOM: fit world to screen width, then apply zoom-in multiplier for a closer view
  const ZOOM=useMemo(()=>{
    return Math.max(0.10, (SW / WORLD_W) * 1.45);
  },[SW]);
...
const[charData,setCharData]=useState(null);
  ...
  useEffect(()=>{
    let alive = true;
    const loadAll = async () => {
      // ── DATA HANDLING: load all persisted state with validation ──
      try{
        ...
        if(raw && alive){
          ...
          }catch(parseErr){ /* corrupt profile — keep defaults */ }
        }
      ...
      try{
        ...
        if(cr && alive){
          try{
            const cd = resolveCharData(JSON.parse(cr));
            if(cd){ charDataRef.current = cd; setCharData(cd); }
          }catch(parseErr){ /* corrupt char data — keep defaults */ }
        }
      }catch(e){}
      ...
      try{
        ...
        if(rc && alive){
          try{
            ...
            if(!isNaN(c) && c >= 0){ totalCoinsRef.current = c; setTotalCoins(c); }
          }catch(parseErr){}
        }
      }catch(e){}
    };
  ...
  const startFlow = useCallback(()=>{
    ...
    console.log('🟦 [startFlow] nameOk:', nameOk, '| hasChar:', hasChar, '| name:', profileRef.current?.name);
    if(!nameOk){ setScreenWithLog('nameset'); return; }
    if(!hasChar){ setScreenWithLog('customize'); return; }
    console.log('✅ [startFlow] Both OK → calling startGame()');
    startGame();
  },[]);
  ...
  const [micPermModalVisible, setMicPermModalVisible] = useState(false);
  ...
  // ── RIDDLE MODAL (shown in elevator between floors) ────────────────────────
  const [riddleVisible, setRiddleVisible] = useState(false);
  const riddleResolveRef = useRef(null); // resolves with earnedCoins when player finishes
  // Use a ref to hold the latest showRiddleChallenge so elevatorSequence never captures a stale closure
  const showRiddleChallengeRef = useRef(null);
  const showRiddleChallenge = useCallback(()=>new Promise(resolve=>{
    riddleResolveRef.current = resolve;
    setRiddleVisible(true);
  }), []);
  // Keep ref always current
  showRiddleChallengeRef.current = showRiddleChallenge;
...
const activateFreeze = useCallback(()=>{
  ...
  useEffect(()=>{
    ...
    const shouldRun = canUseAV && screen==='play' && darkMicRef.current.phase==='active' && micEnabled && Platform.OS !== 'ios';
    const stopMic=async()=>{
    ...
    const startMic=async()=>{
      try{
        ...
        if(!perm.granted){
          setMicEnabled(false);
          setMicPermModalVisible(true); // show styled permission modal
          return;
        }
  ...
  useEffect(()=>{
    ...
    const tick=()=>{
      ...
      if(now-lastRenderEnemies>RENDER_ENEMIES_MS){
        ...
        // Catch any JS error during gameplay to prevent RAF loop from dying on iOS
        // Log it but keep the loop alive
        if(__DEV__) console.warn('[tick error]', tickErr);
      }
  ...
  const startGame=useCallback(()=>{
    console.log('🟢 [startGame] Called — resetting + going to play');
    try {
      resetAll();
      runStartRef.current=Date.now();
      setSilenceAlarm(true);
      setScreenWithLog('play');
      console.log('✅ [startGame] Done');
    } catch(err) {
      console.error('❌ [startGame] CRASHED:', err?.message, err);
    }
  },[resetAll]);
  ...
  const elevatorSequence=useCallback(async()=>{
    ...
    try{
      ...
      // ── MULTIPLE-CHOICE RIDDLE during elevator ride ───────────────────────
      // Use ref to avoid stale closure — showRiddleChallengeRef always points to latest function
      try{
        const questionsEnabled = profileRef.current?.settings?.enableElevatorQuestions !== false;
        const earnedQuestionCoins = questionsEnabled ? await showRiddleChallengeRef.current() : 0;
        if(earnedQuestionCoins > 0){
          const newTotal = totalCoinsRef.current + earnedQuestionCoins;
          totalCoinsRef.current = newTotal;
          setTotalCoins(newTotal);
          try{ await storage.set(COINS_KEY, String(newTotal)); }catch(e){}
        }
      }catch(e){ /* question screen error is non-fatal — continue elevator normally */ }
      // ─────────────────────────────────────────────────────────────────────
  ...
  },[elevatorBusy, finishRun, showMsg]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  },[screen,previewTrigger]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // ── SCREEN ROUTING ─────────────────────────────────────────────────────────
  console.log('🟦 [GameCore] render — screen:', screen, '| boot:', boot, '| name:', profile?.name, '| hasChar:', !!charData, '| Video type:', typeof Video);
  if(screen==='intro') return(
    <>
      <IntroVideoScreen onDone={()=>{ setScreenWithLog(boot?'loading':'hub'); }} />
    </>
  );
  // 2. Loading splash (while boot timer runs, then auto-goes to hub)
  if(screen==='loading'||boot) return(
    <>
      <LaunchScreen onReady={()=>setScreenWithLog('hub')} bootDone={!boot}/>
    </>
  );
  ...
  if(screen==='hub')return(
    <>
      <HubTabs
        ...
        charData={charData}
        onEditCharacter={()=>setScreenWithLog('customize_profile')}
        openSettings={()=>{
          ...
          const handleDeleteAccount = ()=>{
            ...
            setTotalCoins(0);
            const clearedStats = defaultStats;
            setStats(clearedStats);
            statsRef.current=clearedStats;
            setSoundEnabled(true);
            soundEnabledRef.current=true;
            setMicEnabled(MIC_ENABLED_DEFAULT);
            micEnabledRef.current=MIC_ENABLED_DEFAULT;
            resetAll();
            setScreenWithLog('hub');
            return true;
          };
          navigation?.navigate?.('Tabs', { screen: 'Settings', params: { onDeleteAccount: handleDeleteAccount } });
        }}
        openAchievements={()=>navigation?.navigate?.('Tabs', { screen: 'Achievements' })}
      />
    </>
  );
  ...
  if(screen==='customize')return(
    <CharacterCreator
      onDone={async cd=>{
        ...
        }catch(e){ /* non-fatal — char in memory even if save failed */ }
        startGame();
      }}
      uiMsg={uiMsg}
      theme={THEMES[0]}
    ...
    <NameSetup
      ...
      onSave={async nm=>{
        try{
          ...
          updateProfile(next);
          await storage.set(PROFILE_KEY, JSON.stringify(next));
        }catch(e){ /* keep profile in memory even if persist fails */ }
        if(!charDataRef.current){ setScreenWithLog('customize'); return; }
        startGame();
      }}
      onBack={()=>setScreenWithLog('hub')}
    />
  );
  ...
  if(screen==='dead')return(
    <DeadScreen
      ...
      onMenu={()=>setScreenWithLog('hub')}
    />
  );
  if(screen==='win')return<WinScreen charData={charData} durMs={winData.durMs} bestEscapeMs={winData.bestEscapeMs} brokeRecord={winData.brokeRecord} escapes={stats.escapes} onPlay={startGame} onMenu={()=>setScreenWithLog('hub')}/>;
// ── PLAY ─────────────────────────────────────────────────────────────────────
  console.log('🟢 [play screen] About to render — SVG_AVAILABLE:', SVG_AVAILABLE,
    '| Svg type:', typeof Svg,
    '| ChibiCharacter type:', typeof ChibiCharacter,
    '| FloorBackground type:', typeof FloorBackground,
    '| GhostSprite type:', typeof GhostSprite,
    '| GhostZoneRing type:', typeof GhostZoneRing,
    '| WallBlock type:', typeof WallBlock,
    '| PauseModal type:', typeof PauseModal,
    '| HeartRow type:', typeof HeartRow,
    '| CaptureOverlay type:', typeof CaptureOverlay,
    '| ElevatorRideOverlay type:', typeof ElevatorRideOverlay,
  );
  console.log('🟢 [play screen] charData:', charData?._registryId, '| hasSprite:', !!charData?.spriteImage,
    '| cf:', !!currentFloor, '| theme:', !!theme, '| enemyStates.length:', enemyStates.length);
  return(
    <View style={{width:SW, height:SH, backgroundColor:theme.bg, overflow:'hidden', position:'relative'}}>
      ...
      <View style={{
        ...
        <View style={{flexDirection:'row', alignItems:'center', gap:3, flexShrink:0}}>
          ...
          <View
            collapsable={false}
            style={{width:MOVE_BTN_SIZE,height:MOVE_BTN_SIZE,borderRadius:8,
            ...
            onStartShouldSetResponder={()=>!(elevatorBusy||pauseOpen)}
            onResponderGrant={()=>{moveRef.current.left=true;registerTap('left');}}
            onResponderRelease={()=>{moveRef.current.left=false;}}
            onResponderTerminate={()=>{moveRef.current.left=false;}}
          ...
          <View style={{gap:3}}>
            <View
              collapsable={false}
              style={{width:MOVE_BTN_SIZE,height:Math.round(MOVE_BTN_SIZE*0.48),borderRadius:6,
                backgroundColor:theme.edge+'22',borderWidth:1.5,borderColor:theme.edge+'88',
                alignItems:'center',justifyContent:'center',opacity:elevatorBusy?0.3:1}}
              onStartShouldSetResponder={()=>!(elevatorBusy||pauseOpen)}
              onResponderGrant={()=>{moveRef.current.up=true;registerTap('up');}}
              onResponderRelease={()=>{moveRef.current.up=false;}}
              onResponderTerminate={()=>{moveRef.current.up=false;}}
              onResponderTerminationRequest={()=>true}
            >
              <Text style={{color:'white',fontWeight:'900',fontSize:15}}>↑</Text>
            </View>
            <View
              collapsable={false}
              style={{width:MOVE_BTN_SIZE,height:Math.round(MOVE_BTN_SIZE*0.48),borderRadius:6,
                ...
                alignItems:'center',justifyContent:'center',opacity:elevatorBusy?0.3:1}}
              onStartShouldSetResponder={()=>!(elevatorBusy||pauseOpen)}
              onResponderGrant={()=>{moveRef.current.down=true;registerTap('down');}}
              onResponderRelease={()=>{moveRef.current.down=false;}}
              onResponderTerminate={()=>{moveRef.current.down=false;}}
              onResponderTerminationRequest={()=>true}
            >
              <Text style={{color:'white',fontWeight:'900',fontSize:15}}>↓</Text>
            </View>
          ...
          <View
            ...
            style={{width:MOVE_BTN_SIZE,height:MOVE_BTN_SIZE,borderRadius:8,
              ...
              alignItems:'center',justifyContent:'center',opacity:elevatorBusy?0.3:1}}
            onStartShouldSetResponder={()=>!(elevatorBusy||pauseOpen)}
            onResponderGrant={()=>{moveRef.current.right=true;registerTap('right');}}
            onResponderRelease={()=>{moveRef.current.right=false;}}
            onResponderTerminate={()=>{moveRef.current.right=false;}}
            onResponderTerminationRequest={()=>true}
          >
            <Text style={{color:'white',fontWeight:'900',fontSize:18}}>→</Text>
          </View>
        ...
        {/* Thin divider */}
        <View style={{width:1,height:MOVE_BTN_SIZE*0.7,backgroundColor:theme.edge+'44'}}/>
        ...
        <View style={{flex:1, flexDirection:'row', gap:4, alignItems:'center'}}>
          ...
          <TouchableOpacity
            ...
            <Text style={{color:inv.freezeCharges>0&&!freezeActive?'#88ddff':freezeActive?'#55ccff':'rgba(100,180,220,0.38)',
              ...
              FREEZE {inv.freezeCharges}{freezeActive?' ON':''}
            </Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity
            ...
            <Text style={{color:inv.shieldCharges>0?theme.accent:'rgba(255,255,255,0.25)',
              fontWeight:'900', fontSize:9, fontFamily:'monospace'}}>
              SHIELD {inv.shieldCharges}{inv.shieldArmed?' ON':''}
            </Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity
            ...
            <Text style={{
              ...
              {inElevator&&allKeys?'OPEN DOOR':'LIFT'}
            </Text>
          </TouchableOpacity>
        </View>
      ...
      {/* ── Elevator Riddle Modal — appears during elevator ride as multiple choice ── */}
      <Modal visible={riddleVisible} transparent animationType="fade" statusBarTranslucent>
        <GhostRiddleScreen
          onDone={(earnedCoins)=>{
            setRiddleVisible(false);
            if(riddleResolveRef.current){
              riddleResolveRef.current(earnedCoins);
              riddleResolveRef.current = null;
            }
          }}
        />
      </Modal>

      {/* ── MIC PERMISSION REQUIRED MODAL ─────────────────────────────────── */}
      <Modal visible={micPermModalVisible} transparent animationType="fade" statusBarTranslucent>
        <View style={{flex:1,backgroundColor:'rgba(0,0,0,0.80)',alignItems:'center',justifyContent:'center',padding:28}}>
          <View style={{
            width:'100%',maxWidth:380,borderRadius:20,padding:22,
            backgroundColor:'rgba(10,5,25,0.97)',
            borderWidth:1.5,borderColor:'rgba(180,100,255,0.40)',
          }}>
            {/* Icon */}
            <View style={{alignItems:'center',marginBottom:14}}>
              <View style={{width:58,height:58,borderRadius:29,
                backgroundColor:'rgba(180,100,255,0.15)',borderWidth:1.5,borderColor:'rgba(180,100,255,0.40)',
                alignItems:'center',justifyContent:'center',marginBottom:8}}>
                <Text style={{fontSize:26}}>🎙</Text>
              </View>
              <Text style={{color:'#d6c8ff',fontFamily:'monospace',fontWeight:'900',fontSize:14,letterSpacing:3}}>
                MICROPHONE NEEDED
              </Text>
            </View>
            {/* Explanation */}
            <Text style={{color:'rgba(220,200,255,0.75)',fontFamily:'monospace',fontSize:11,lineHeight:18,textAlign:'center',marginBottom:10}}>
              Dark Mode uses your microphone to detect sound.{'\n'}
              Ghosts react to loud noises you make — silence is your only weapon.
            </Text>
            <Text style={{color:'rgba(255,180,100,0.70)',fontFamily:'monospace',fontSize:10,lineHeight:16,textAlign:'center',marginBottom:18}}>
              To enable: go to your device{'\n'}
              <Text style={{fontWeight:'900'}}>Settings → Apps → [This App] → Permissions → Microphone</Text>
            </Text>
            {/* Buttons */}
            <View style={{gap:10}}>
              <TouchableOpacity
                onPress={async()=>{
                  ...
                  try{
                    ...
                    if(perm.granted){
                      setMicEnabled(true);
                      setMicPermModalVisible(false);
                    } else {
                      // Still denied — keep modal open with message
                      Alert.alert('Still Denied','Please enable microphone in your device Settings app.');
                    }
                  }catch(e){ setMicPermModalVisible(false); }
                }}
                activeOpacity={0.85}
                style={{paddingVertical:13,borderRadius:12,alignItems:'center',
                  backgroundColor:'rgba(180,100,255,0.18)',borderWidth:1.5,borderColor:'rgba(180,100,255,0.55)'}}>
                <Text style={{color:'#d6c8ff',fontFamily:'monospace',fontWeight:'900',fontSize:12,letterSpacing:2}}>
                  ↺ RETRY PERMISSION
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                onPress={()=>{
                  setMicEnabled(false);
                  setMicPermModalVisible(false);
                  showMsg('🎙 Mic disabled — Dark Mode sound detection off.');
                }}
                activeOpacity={0.8}
                style={{paddingVertical:12,borderRadius:12,alignItems:'center',
                  backgroundColor:'rgba(255,255,255,0.03)',borderWidth:1,borderColor:'rgba(255,255,255,0.12)'}}>
                <Text style={{color:'rgba(255,255,255,0.40)',fontFamily:'monospace',fontSize:11,letterSpacing:2}}>
...
function ResultsScreen({ route, navigation }){
  ...
  const fade = useRef(new Animated.Value(0)).current;
  useEffect(()=>{
    const anim = Animated.timing(fade,{toValue:1,duration:600,useNativeDriver:true});
    anim.start();
    return ()=>{ anim.stop(); fade.stopAnimation(); };
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[]);
  return (
...
function AchievementsScreen({ navigation }){
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      // Load saved location
      try{
        // LOCATION_KEY is 'LASTFLOOR_LOCATION_V2' — imported from storage utils
        const sl = await storage.get(LOCATION_KEY);
  ...
  const requestLocation = async()=>{
    ...
    try{
      ...
      await storage.set(LOCATION_KEY, JSON.stringify(loc));
...
function SettingsScreen({ navigation, route }){
  ...
  const [soundEnabled, setSoundEnabled] = useState(true);
  const [elevatorQuestionsEnabled, setElevatorQuestionsEnabled] = useState(true);
  const [deleteConfirm, setDeleteConfirm] = useState(false);
  const [deleting, setDeleting] = useState(false);
  const fadeAnim = useRef(new Animated.Value(0)).current;
  // onDeleteAccount is passed via navigation params from GameCore → 'Settings' Stack screen
  const onDeleteAccount = route?.params?.onDeleteAccount ?? null;

  useEffect(()=>{
    const anim = Animated.timing(fadeAnim,{toValue:1,duration:500,useNativeDriver:true});
    anim.start();
    let alive=true;
    (async()=>{
      try{
        ...
        if(raw && alive){
          ...
          const q = j?.settings?.enableElevatorQuestions;
          if(typeof v === 'boolean') setMic(v);
          if(typeof s === 'boolean') setSoundEnabled(s);
          if(typeof q === 'boolean') setElevatorQuestionsEnabled(q);
        }
      }catch(e){}
    })();
    return ()=>{ alive=false; anim.stop(); fadeAnim.stopAnimation(); };
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[]);
  ...
  const saveElevatorQuestions = async(nextValue)=>{ setElevatorQuestionsEnabled(nextValue); await saveSetting('enableElevatorQuestions', nextValue); };

  const performDeleteAccount = async()=>{
    if(deleting) return;
    setDeleting(true);
    try{
      await clearAllGameStorage();
      ...
      setDeleteConfirm(false);
      console.warn('Could not delete account', err);
    }finally{
      setDeleting(false);
    }
  };
  ...
  const ToggleSwitch = ({label, desc, value, onChange, colorOn='#5ecfa0', colorOff='rgba(255,255,255,0.18)'})=>{
    ...
    useEffect(()=>{
      const anim = Animated.spring(knob,{toValue:value?1:0,friction:6,tension:100,useNativeDriver:false});
      anim.start();
      return ()=>{ anim.stop(); };
    // eslint-disable-next-line react-hooks/exhaustive-deps
    },[value]);
        ...
        <ScrollView contentContainerStyle={{padding:20,paddingBottom:40}} showsVerticalScrollIndicator={false}>
          ...
          <View style={{flexDirection:'row',alignItems:'center',justifyContent:'space-between',marginBottom:24,paddingTop:4}}>
            ...
            <TouchableOpacity
              ...
              <Text style={{color:'#e8dcc8',fontFamily:'monospace',fontWeight:'900',fontSize:10,letterSpacing:2}}>← BACK</Text>
            </TouchableOpacity>
          </View>
          ...
          <ToggleSwitch
            label="ELEVATOR QUESTIONS"
            desc={"When enabled, the elevator stops for a local multiple-choice question. Turn it off to skip directly to the next floor."}
            value={elevatorQuestionsEnabled}
            onChange={saveElevatorQuestions}
            colorOn='#a090ff'
          />
          ...
          <TouchableOpacity
            ...
            style={{
              ...
              backgroundColor: deleteConfirm ? 'rgba(255,30,30,0.14)' : 'rgba(255,30,30,0.06)',
              borderWidth:1.5,borderColor: deleteConfirm ? 'rgba(255,120,120,0.65)' : 'rgba(255,80,80,0.35)',
              alignItems:'center',marginBottom:8,
            }}>
            <Text style={{color:'#ff5566',fontFamily:'monospace',fontWeight:'900',fontSize:12,letterSpacing:3}}>
              {deleting ? 'DELETING...' : deleteConfirm ? 'PRESS AGAIN TO DELETE' : 'DELETE ACCOUNT'}
            </Text>
            <Text style={{color:'rgba(255,120,120,0.5)',fontFamily:'monospace',fontSize:9,marginTop:5,letterSpacing:1,textAlign:'center'}}>
              {deleteConfirm
                ? 'Second press will permanently remove all profile, coins, stats and progress'
...
export { SettingsScreen, AchievementsScreen };

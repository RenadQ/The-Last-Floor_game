...
try {
  ...
  if (typeof candidate === 'function') {
    ...
    Video = null;
    console.warn('⚠️ [expo-av] Video is NOT a function — disabling to prevent crash');
  }
} catch (e) {
...
const[charData,setCharData]=useState(null);
  ...
  const currentFloor = floorsRef.current[floorIndex] ?? floorsRef.current[0];
  const theme = THEMES[currentFloor?.theme] ?? THEMES[0];
...
const activateFreeze = useCallback(()=>{
  ...
  const startGame=useCallback(()=>{
    ...
    try {
      ...
      console.log('✅ [startGame] Done — screen set to play');
    } catch(err) {
  ...
  // ── SCREEN ROUTING ─────────────────────────────────────────────────────────
  console.log('🟦 [GameCore] render — screen:', screen, '| boot:', boot, '| name:', profile?.name, '| hasChar:', !!charData, '| floorIndex:', floorIndex, '| theme:', theme?.bg ? 'OK' : '❌ UNDEFINED');
...
// ── PLAY ─────────────────────────────────────────────────────────────────────
  console.log('🟢 [play screen] Rendering — theme:', theme?.bg, '| currentFloor:', !!currentFloor, '| cf.walls:', currentFloor?.walls?.length);
  return(

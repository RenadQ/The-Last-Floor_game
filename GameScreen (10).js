...
// Safe import for expo-av (Audio only)
let Audio;
try {
  const avModule = require('expo-av');
  Audio = avModule.Audio;
  console.log('✅ expo-av Audio loaded');
} catch (e) {
  Audio = null;
  console.warn('⚠️ expo-av not available:', e.message);
}

// Safe import for Video — validate it's actually a renderable component
let Video = null;
let ResizeMode = null;
try {
  const avModule = require('expo-av');
  const candidate = avModule.Video;
  // Must be a function or class — not a plain object
  if (typeof candidate === 'function') {
    Video = candidate;
    ResizeMode = avModule.ResizeMode;
    console.log('✅ expo-av Video loaded OK');
  } else {
    console.warn('⚠️ expo-av Video is type:', typeof candidate, '— not a valid component, disabling video playback');
    Video = null;
  }
} catch (e) {
  console.warn('⚠️ expo-av Video import failed:', e.message);
  Video = null;
}
...
const activateFreeze = useCallback(()=>{
  ...
  if(screen==='win')return<WinScreen charData={charData} durMs={winData.durMs} bestEscapeMs={winData.bestEscapeMs} brokeRecord={winData.brokeRecord} escapes={stats.escapes} onPlay={startGame} onMenu={()=>setScreen('hub')}/>;
// ── PLAY ─────────────────────────────────────────────────────────────────────
  return(
